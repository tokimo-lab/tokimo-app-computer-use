...
pub enum ScreenshotAction {
  Desktop { #[arg(long, default_value = "webp")] format: String, #[arg(long, default_value = "80")] quality: u8, #[arg(long)] output: Option<String> },
  Window { #[arg(long, short = 'w')] handle: Option<i64>, #[arg(long, default_value = "webp")] format: String, #[arg(long, default_value = "80")] quality: u8, #[arg(long)] output: Option<String> },
}
...
pub fn cmd(executor: &mut dyn CommandExecutor, action: ScreenshotAction) -> Result<()> {
  match action {
    ...
    ScreenshotAction::Window { handle, format, quality, output } => {
...
fn save_screenshot(result: &serde_json::Value, output: Option<String>, format: &str) -> Result<()> {
  let data_b64 = result.get("data").and_then(|v| v.as_str())
