...
pub fn cmd(executor: &mut dyn CommandExecutor, action: UsbAction) -> Result<()> {
  ...
  match action {
    UsbAction::List => {
      let Some(arr) = usbs else { println!("No USB devices found."); return Ok(()) };
      if arr.is_empty() { println!("No USB devices found."); return Ok(()); }
      ...
      for (i, u) in arr.iter().enumerate() {
        ...
        let id = if !vid.is_empty() { format!("{vid}:{pid}") } else { "-".to_string() };
        t.row(vec![i.to_string(), name.to_string(), id, mfr.to_string()]);
      }
      t.print();
    }
    UsbAction::Detail { index } => {
      let Some(arr) = usbs else { anyhow::bail!("No USB devices found") };
      let u = arr.get(index).ok_or_else(|| anyhow::anyhow!("USB device index {index} out of range (0..{})", arr.len()))?;
      super::kv_print(&[
