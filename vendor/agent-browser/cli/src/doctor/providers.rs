...
pub(super) fn check(checks: &mut Vec<Check>) {
  ...
  let normalized = active.as_ref().map(|s| s.to_lowercase()).unwrap_or_default();
  ...
  let aws_present =
    env::var("AWS_ACCESS_KEY_ID").is_ok() || env::var("AWS_PROFILE").is_ok() || env::var("AWS_SESSION_TOKEN").is_ok();
  ...
  if agentcore_status == Status::Fail {
    agentcore_check = agentcore_check.with_fix("export AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY or AWS_PROFILE");
  }
