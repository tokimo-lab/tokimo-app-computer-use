...
pub(super) fn check(checks: &mut Vec<Check>) {
  ...
  if env::var("AI_GATEWAY_API_KEY").is_ok() {
    let url = env::var("AI_GATEWAY_URL").unwrap_or_else(|_| "https://ai-gateway.vercel.sh".to_string());
    probe_url(&rt, &client, checks, category, "net.ai_gateway", &url, "AI Gateway");
  }
  ...
  if let Ok(provider) = env::var("AGENT_BROWSER_PROVIDER") {
    let url: Option<String> = match provider.to_lowercase().as_str() {
      ...
      "browserless" => {
        Some(env::var("BROWSERLESS_API_URL").unwrap_or_else(|_| "https://production-sfo.browserless.io".to_string()))
      }
      "browseruse" | "browser-use" => Some("https://api.browser-use.com".to_string()),
      "kernel" => Some(env::var("KERNEL_ENDPOINT").unwrap_or_else(|_| "https://api.onkernel.com".to_string())),
      _ => None,
    };
    if let Some(url) = url {
      probe_url(
    ...
    Ok(resp) => {
      ...
      if status.is_success() || status.is_redirection() || status.as_u16() == 405 {
        checks.push(Check::new(
          ...
          category,
          Status::Pass,
          format!("{} reachable ({}ms, HTTP {})", label, elapsed_ms, status.as_u16()),
        ));
      } else {
        checks.push(Check::new(
          id,
          category,
          Status::Warn,
          format!("{} returned HTTP {} after {}ms", label, status.as_u16(), elapsed_ms),
        ));
      }
    }
    Err(e) => {
