...
pub fn find_installed_chrome() -> Option<PathBuf> {
  ...
  let mut versions: Vec<_> = entries
    .filter_map(|e| e.ok())
    .filter(|e| {
      let matches = e.file_name().to_str().is_some_and(|n| n.starts_with("chrome-"));
      if debug {
        let _ = writeln!(
          io::stderr(),
  ...
  for entry in versions {
    ...
    if let Some(bin) = chrome_binary_in_dir(&dir) {
      ...
      let _ = writeln!(io::stderr(), "[chrome-search] no binary found in {}", dir.display());
    }
  }
...
async fn download_bytes(url: &str) -> Result<Vec<u8>, String> {
  ...
  for attempt in 0..max_retries {
    if attempt > 0 {
      eprintln!("  Retrying download (attempt {}/{})", attempt + 1, max_retries);
    ...
    if !status.is_success() {
      last_err = format!("Download failed: server returned HTTP {} for {}", status, url);
...
fn extract_zip(bytes: Vec<u8>, dest: &Path) -> Result<(), String> {
  ...
  let mut archive = zip::ZipArchive::new(cursor).map_err(|e| format!("Failed to read zip archive: {}", e))?;
  ...
  for i in 0..archive.len() {
    ...
    if file.is_dir() {
      ...
      if let Some(parent) = out_path.parent() {
        fs::create_dir_all(parent).map_err(|e| format!("Failed to create parent dir {}: {}", parent.display(), e))?;
      }
      let mut out_file =
        fs::File::create(&out_path).map_err(|e| format!("Failed to create file {}: {}", out_path.display(), e))?;
      io::copy(&mut file, &mut out_file).map_err(|e| format!("Failed to write {}: {}", out_path.display(), e))?;
...
pub fn run_install(with_deps: bool) {
  ...
  let rt = tokio::runtime::Builder::new_current_thread()
    ...
    .build()
    .unwrap_or_else(|e| {
      eprintln!("{} Failed to create runtime: {}", color::error_indicator(), e);
      exit(1);
    });
...
fn install_linux_deps() {
  ...
  if pkg_mgr == "apt-get" {
    ...
    match update_status {
      ...
      Err(e) => {
        eprintln!("{} Could not run apt-get update: {}", color::warning_indicator(), e);
      }
      _ => {}
    }
    ...
    match sim_output {
      Ok(output) => {
        ...
        let removals: Vec<&str> = combined.lines().filter(|line| line.starts_with("Remv ")).collect();
        ...
        if !removals.is_empty() {
          ...
          eprintln!("  This usually means some package names have changed on your system");
...
mod tests {
  ...
  async fn download_bytes_returns_error_on_404() {
    ...
    assert!(err.contains("HTTP 404"), "expected HTTP 404 in error, got: {}", err);
    server.await.unwrap();
  }
  ...
  async fn download_bytes_gives_up_after_max_retries() {
    ...
    assert!(result.is_err());
    let err = result.unwrap_err();
    assert!(err.contains("HTTP 500"), "expected HTTP 500 in error, got: {}", err);
    server.await.unwrap();
  }
  ...
  fn download_bytes_connection_refused_includes_details() {
    ...
    assert!(
      err.contains("Connection refused") || err.contains("connection refused") || err.contains("actively refused it"),
