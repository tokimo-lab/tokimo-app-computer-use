  ...
  pub fn format(&self) -> String {
    match self {
      ...
      ParseError::MissingArguments { context, usage } => {
        format!("Missing arguments for: {}\nUsage: agent-browser {}", context, usage)
      }
...
pub fn parse_curl_cookies(raw: &str) -> Result<Vec<Value>, String> {
  ...
  if trimmed.starts_with('[') {
    let arr: Vec<Value> = serde_json::from_str(trimmed).map_err(|e| format!("cookies JSON parse error: {}", e))?;
  ...
  match cmd {
    ...
    "open" | "goto" | "navigate" => {
      ...
      if let Some(ref headers_json) = flags.headers {
        let headers =
          serde_json::from_str::<serde_json::Value>(headers_json).map_err(|_| ParseError::InvalidValue {
    ...
    "screenshot" => {
      ...
      let (selector, path) = match (positional.first(), positional.get(1)) {
        ...
        (Some(first), None) => {
          ...
          let is_selector =
            !is_relative_path && (first.starts_with('.') || first.starts_with('#') || first.starts_with('@'));
          let has_path_extension =
            first.ends_with(".png") || first.ends_with(".jpg") || first.ends_with(".jpeg") || first.ends_with(".webp");
        ...
        Some("save") => {
          ...
          if !password_stdin && password.is_none() {
            return Err(ParseError::MissingArguments {
              context: "auth save".to_string(),
              usage:
                "agent-browser auth save <name> --url <url> --username <user> --password <pass> [--password-stdin]",
            });
          }
    ...
    "connect" => {
        ...
        || endpoint.starts_with("https://")
      {
        ...
        let port: u16 = match endpoint.parse::<u32>() {
          ...
          Ok(p) if p > 65535 => {
            return Err(ParseError::InvalidValue {
              message: format!("Invalid port: {} is out of range (valid range: 1-65535)", p),
              usage: "connect <port|url>",
            });
          }
          Ok(p) => p as u16,
          Err(_) => {
            return Err(ParseError::InvalidValue {
              message: format!("Invalid value: '{}' is not a valid port number or URL", endpoint),
              usage: "connect <port|url>",
            });
          }
        };
          ...
          match rest[i] {
            "--port" => {
              ...
              if port > u16::MAX as u32 {
                return Err(ParseError::InvalidValue {
                  message: format!("Invalid port: {} is out of range (valid range: 0-65535)", port),
                  usage: "stream enable [--port <port>]",
                });
              }
              cmd["port"] = json!(port);
      ...
      match *op {
        "set" => {
          ...
          if let Some(curl_idx) = rest.iter().position(|a| *a == "--curl") {
            let path = rest.get(curl_idx + 1).ok_or_else(|| ParseError::MissingArguments {
              context: "cookies set --curl".to_string(),
              usage: "cookies set --curl <file> [--domain <domain>] [--url <url>]",
            })?;
            let raw = std::fs::read_to_string(path).map_err(|e| ParseError::InvalidValue {
              message: format!("cookies --curl: cannot read '{}': {}", path, e),
              usage: "cookies set --curl <file>",
            })?;
            let mut cookies = parse_curl_cookies(&raw).map_err(|e| ParseError::InvalidValue {
    ...
    "clipboard" => match rest.first().copied() {
      Some("read") | None => Ok(json!({ "id": id, "action": "clipboard", "operation": "read" })),
      Some("write") => {
        ...
        match rest[i] {
          ...
          other if other.starts_with('-') => {
            return Err(ParseError::InvalidValue {
              message: format!("Unknown flag: {}", other),
              usage:
                "diff screenshot --baseline <file> [--output <file>] [--threshold <0-1>] [--selector <sel>] [--full/-f]",
            });
          }
          other => {
            return Err(ParseError::InvalidValue {
  ...
  match *locator {
    "role" | "text" | "label" | "placeholder" | "alt" | "title" | "testid" | "first" | "last" => {
      ...
      if rest.len() > 3 {
        ...
        while i < rest.len() {
          match rest[i] {
            ...
            "--name" => {
              let n = rest.get(i + 1).ok_or_else(|| ParseError::MissingArguments {
                context: format!("find {}", locator),
                usage: "find role <role> [action] [--name <name>] [--exact]",
              })?;
              name = Some(*n);
              i += 2;
            }
      ...
      match *locator {
        ...
        "label" => {
          let mut cmd =
            json!({ "id": id, "action": "getbylabel", "label": value, "subaction": subaction, "exact": exact });
          if let Some(v) = fill_value {
            cmd["value"] = json!(v);
          }
          Ok(cmd)
        ...
        "alt" => {
          Ok(json!({ "id": id, "action": "getbyalttext", "text": value, "subaction": subaction, "exact": exact }))
        }
        "title" => {
          Ok(json!({ "id": id, "action": "getbytitle", "text": value, "subaction": subaction, "exact": exact }))
        }
        "testid" => {
...
fn parse_mouse(rest: &[&str], id: &str) -> Result<Value, ParseError> {
  ...
  match rest.first().copied() {
    ...
    Some("down") => Ok(json!({ "id": id, "action": "mousedown", "button": rest.get(1).unwrap_or(&"left") })),
    Some("up") => Ok(json!({ "id": id, "action": "mouseup", "button": rest.get(1).unwrap_or(&"left") })),
    Some("wheel") => {
  ...
  match rest.first().copied() {
    Some("viewport") => {
      ...
      if let Some(scale_str) = rest.get(3) {
        let scale = scale_str.parse::<f64>().map_err(|_| ParseError::MissingArguments {
          context: "set viewport".to_string(),
          usage: "set viewport <width> <height> [scale]",
        })?;
...
fn parse_network(rest: &[&str], id: &str) -> Result<Value, ParseError> {
  ...
  match rest.first().copied() {
    Some("route") => {
      ...
      let mut cmd = json!({ "id": id, "action": "route", "url": url, "abort": abort, "body": body });
...
mod tests {
  ...
  fn test_react_renders_start_stop() {
    ...
    let stop_json = parse_command(&args("react renders stop --json"), &default_flags()).unwrap();
  ...
  fn test_react_suspense_only_dynamic_with_json() {
    let cmd = parse_command(&args("react suspense --only-dynamic --json"), &default_flags()).unwrap();
    assert_eq!(cmd["action"], "react_suspense");
    assert_eq!(cmd["onlyDynamic"], true);
    assert_eq!(cmd["json"], true);
  }
  ...
  fn test_vitals_command() {
    ...
    let cmd = parse_command(&args("vitals http://localhost:3000/dashboard"), &default_flags()).unwrap();
  ...
  fn test_cookies_set_with_multiple_flags() {
    let cmd = parse_command(
      &args("cookies set mycookie myvalue --url https://example.com --httpOnly --secure --sameSite Lax"),
      &default_flags(),
    )
    .unwrap();
    assert_eq!(cmd["action"], "cookies_set");
  ...
  fn test_set_headers_invalid_json_error() {
    let input: Vec<String> = vec!["set".to_string(), "headers".to_string(), "not-valid-json".to_string()];
  ...
  fn test_tab_new_with_label_and_url() {
    let cmd = parse_command(&args("tab new --label docs https://docs.example.com"), &default_flags()).unwrap();
    assert_eq!(cmd["action"], "tab_new");
    assert_eq!(cmd["label"], "docs");
  ...
  fn test_tab_new_with_url_then_label() {
    let cmd = parse_command(&args("tab new https://docs.example.com --label docs"), &default_flags()).unwrap();
    assert_eq!(cmd["url"], "https://docs.example.com");
    assert_eq!(cmd["label"], "docs");
  }
  ...
  fn test_tab_unknown_flag_errors() {
    ...
    let result = parse_command(&args("tab new --unknown-flag https://example.com"), &default_flags());
    assert!(
      result.is_err(),
  ...
  fn test_network_requests_type_filter() {
    let cmd = parse_command(&args("network requests --type xhr,fetch"), &default_flags()).unwrap();
  ...
  fn test_wait_text_with_timeout() {
    let cmd = parse_command(&args("wait --text Welcome --timeout 5000"), &default_flags()).unwrap();
    assert_eq!(cmd["action"], "wait");
    assert_eq!(cmd["text"], "Welcome");
    assert_eq!(cmd["timeout"], 5000);
  }
  ...
  fn test_eval_base64_long_flag() {
    // "document.title" in base64
    let cmd = parse_command(&args("eval --base64 ZG9jdW1lbnQudGl0bGU="), &default_flags()).unwrap();
    assert_eq!(cmd["action"], "evaluate");
    assert_eq!(cmd["script"], "document.title");
  }
  ...
  fn test_diff_snapshot_baseline() {
    let cmd = parse_command(&args("diff snapshot --baseline before.txt"), &default_flags()).unwrap();
    assert_eq!(cmd["action"], "diff_snapshot");
  ...
  fn test_diff_snapshot_short_flags() {
    let cmd = parse_command(&args("diff snapshot -b snap.txt -s .content -c -d 2"), &default_flags()).unwrap();
    assert_eq!(cmd["action"], "diff_snapshot");
  ...
  fn test_diff_screenshot_baseline() {
    let cmd = parse_command(&args("diff screenshot --baseline before.png"), &default_flags()).unwrap();
  ...
  fn test_diff_screenshot_command_full_flag() {
    let cmd = parse_command(&args("diff screenshot --baseline b.png --full"), &default_flags()).unwrap();
    assert_eq!(cmd["action"], "diff_screenshot");
    assert_eq!(cmd["fullPage"], true);
  }
  ...
  fn test_diff_screenshot_command_full_flag_shorthand() {
    let cmd = parse_command(&args("diff screenshot --baseline b.png -f"), &default_flags()).unwrap();
    assert_eq!(cmd["action"], "diff_screenshot");
    assert_eq!(cmd["fullPage"], true);
  }
  ...
  fn test_diff_url_basic() {
    let cmd = parse_command(&args("diff url https://a.com https://b.com"), &default_flags()).unwrap();
  ...
  fn test_diff_url_command_full_flag() {
    let cmd = parse_command(&args("diff url https://a.com https://b.com --full"), &default_flags()).unwrap();
    assert_eq!(cmd["fullPage"], true);
  }
  ...
  fn test_diff_screenshot_threshold_missing_value() {
    let result = parse_command(&args("diff screenshot --baseline b.png --threshold"), &default_flags());
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), ParseError::MissingArguments { .. }));
  }
  ...
  fn test_diff_screenshot_output_missing_value() {
    let result = parse_command(&args("diff screenshot --baseline b.png --output"), &default_flags());
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), ParseError::MissingArguments { .. }));
  }
  ...
  fn test_diff_screenshot_unexpected_arg() {
    let result = parse_command(&args("diff screenshot --baseline b.png unexpected"), &default_flags());
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), ParseError::InvalidValue { .. }));
  }
  ...
  fn test_diff_url_unexpected_arg() {
    let result = parse_command(&args("diff url https://a.com https://b.com extra"), &default_flags());
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), ParseError::InvalidValue { .. }));
  }
  ...
  fn test_scroll_selector_before_positional() {
    let cmd = parse_command(&args("scroll --selector .panel down 400"), &default_flags()).unwrap();
    assert_eq!(cmd["action"], "scroll");
    assert_eq!(cmd["direction"], "down");
