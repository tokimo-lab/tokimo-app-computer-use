...
fn detect_install_method() -> InstallMethod {
  if let Ok(exe) = std::env::current_exe() {
    ...
    if path_str.contains("node_modules/agent-browser") || path_str.contains("node_modules\\agent-browser") {
  ...
  if command_output_contains("pnpm", &["list", "-g", "agent-browser", "--depth=0"], "agent-browser") {
    return InstallMethod::Pnpm;
  }
...
fn run_upgrade_command(method: &InstallMethod) -> bool {
  let (cmd, args, display): (&str, &[&str], &str) = match method {
    ...
    InstallMethod::Homebrew => ("brew", &["upgrade", "agent-browser"], "brew upgrade agent-browser"),
    InstallMethod::Cargo => (
      "cargo",
...
pub fn run_upgrade() {
  ...
  let latest = match rt.block_on(fetch_latest_version()) {
    Ok(v) => v,
    Err(e) => {
      eprintln!("{} Could not check latest version: {}", color::warning_indicator(), e);
      String::new()
    }
  };
  ...
  if matches!(method, InstallMethod::Unknown) {
    eprintln!("{} Could not detect installation method.", color::error_indicator());
  ...
  if !latest.is_empty() {
    println!(
      "{}",
      color::cyan(&format!("Upgrading agent-browser... v{} → v{}", current, latest))
    );
  } else {
    println!("{}", color::cyan(&format!("Upgrading agent-browser (v{})...", current)));
  }
  ...
  if success {
    if !latest.is_empty() {
      println!("{} Done! v{} → v{}", color::success_indicator(), current, latest);
    } else {
