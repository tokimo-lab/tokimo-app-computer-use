...
fn find_skills_dirs() -> Vec<PathBuf> {
  ...
  SKILL_DIRS.iter().map(|d| root.join(d)).filter(|p| p.is_dir()).collect()
}
...
fn run_list(skills_dirs: &[PathBuf], json_mode: bool) {
  let skills: Vec<SkillInfo> = discover_skills(skills_dirs).into_iter().filter(|s| !s.hidden).collect();
  if skills.is_empty() {
    if json_mode {
      println!(
        "{}",
...
fn run_get(skills_dirs: &[PathBuf], names: &[String], get_all: bool, full: bool, json_mode: bool) {
  ...
  let targets: Vec<&SkillInfo> = if get_all {
    ...
    for name in names {
      if name.starts_with('-') {
        eprintln!("{} Unknown flag ignored: {}", color::warning_indicator(), name);
        continue;
      }
...
fn run_path(skills_dirs: &[PathBuf], name: Option<&str>, json_mode: bool) {
  match name {
    ...
    None => {
      let paths: Vec<String> = skills_dirs.iter().map(|d| d.to_string_lossy().to_string()).collect();
      if json_mode {
        println!(
          "{}",
          serde_json::to_string(&json!({
  ...
  match subcommand {
    ...
    Some(unknown) => {
      if json_mode {
        ...
        eprintln!("{} Unknown skills subcommand: {}", color::error_indicator(), unknown);
      }
      exit(1);
    }
  }
