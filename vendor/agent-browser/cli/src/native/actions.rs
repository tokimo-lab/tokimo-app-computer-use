...
use super::cdp::types::{
  AttachToTargetParams, AttachToTargetResult, CdpEvent, CreateTargetResult, DispatchMouseEventParams,
  ExceptionThrownEvent, JavascriptDialogOpeningEvent, TargetCreatedEvent, TargetDestroyedEvent, TargetInfoChangedEvent,
};
use super::cookies;
use super::diff;
...
impl DaemonState {
      ...
      loop {
        match rx.recv().await {
          Ok(event) if event.method == "Page.javascriptDialogOpening" => {
            let dialog_type = event.params.get("type").and_then(|v| v.as_str()).unwrap_or("");
            if matches!(dialog_type, "beforeunload" | "alert") {
              ...
              if let Err(e) = client
                ...
                .await
              {
                eprintln!("[auto-dismiss] failed to dismiss {} dialog: {}", dialog_type, e);
              }
            }
          }
          Ok(_) => continue,
  ...
  pub async fn update_stream_client(&self) {
    ...
    if let Some(ref server) = self.stream_server {
      ...
      server.broadcast_status(connected, sc, vw, vh, &self.engine).await;
  ...
  async fn start_recording_task(&mut self, client: Arc<CdpClient>, session_id: String) -> Result<(), String> {
  ...
  async fn apply_drained_events(&mut self, drained: DrainedEvents) {
    ...
    for (frame_id, iframe_sid) in &drained.attached_iframe_sessions {
      self.iframe_sessions.insert(frame_id.clone(), iframe_sid.clone());
      if let Some(ref mgr) = self.browser {
        let _ = mgr
          .client
          .send_command_no_params("Runtime.runIfWaitingForDebugger", Some(iframe_sid.as_str()))
          .await;
        let _ = mgr
          .client
        ...
        Ok(event) => {
          ...
          match event.method.as_str() {
            ...
            "Target.targetInfoChanged" => {
              if let Ok(te) = serde_json::from_value::<TargetInfoChangedEvent>(event.params.clone()) {
                if should_track_target(&te.target_info) {
            ...
            "Target.targetDestroyed" => {
              if let Ok(te) = serde_json::from_value::<TargetDestroyedEvent>(event.params.clone()) {
              ...
              if let (Some(sid), Some(target_info)) = (
                ...
                if target_type == "iframe" {
                  ...
                  if let Some(target_id) = target_info.get("targetId").and_then(|v| v.as_str()) {
                    attached_iframe_sessions.push((target_id.to_string(), sid.to_string()));
                  }
                }
              }
              continue;
            ...
            "Target.detachedFromTarget" => {
              if let Some(sid) = event.params.get("sessionId").and_then(|v| v.as_str()) {
          ...
          match event.method.as_str() {
            "Runtime.consoleAPICalled" => {
              let level = event.params.get("type").and_then(|v| v.as_str()).unwrap_or("log");
            ...
            "Runtime.exceptionThrown" => {
              if let Ok(ex_event) = serde_json::from_value::<ExceptionThrownEvent>(event.params.clone()) {
                ...
                self
                  .event_tracker
                  .add_error(text, None, details.line_number, details.column_number);
                if let Some(ref server) = self.stream_server {
                  server.broadcast_page_error(text, details.line_number, details.column_number);
                }
              }
            }
            "Network.requestWillBeSent" if self.har_recording || self.request_tracking => {
              if let Some(request) = event.params.get("request") {
                ...
                if self.har_recording {
                  let wall_time = event.params.get("wallTime").and_then(|v| v.as_f64()).unwrap_or(0.0);
                  let request_headers = har_extract_headers(request.get("headers"));
                  let post_data = request.get("postData").and_then(|v| v.as_str()).map(String::from);
                ...
                if self.request_tracking {
                  let headers = request.get("headers").cloned().unwrap_or(json!({}));
                  let resource_type = event
                    .params
                    .get("type")
                    .and_then(|v| v.as_str())
                  ...
                  self.tracked_requests.push(TrackedRequest {
                    ...
                    resource_type,
                    request_id,
                    post_data: request.get("postData").and_then(|v| v.as_str()).map(String::from),
                    status: None,
            ...
            "Network.responseReceived" if self.har_recording || self.request_tracking => {
              if let Some(response) = event.params.get("response") {
                let request_id = event.params.get("requestId").and_then(|v| v.as_str()).unwrap_or("");
                ...
                let redirect_url = response_headers
                  ...
                  .map(|(_, v)| v.clone())
                  .unwrap_or_default();
                let encoded_data_length = response.get("encodedDataLength").and_then(|v| v.as_i64()).unwrap_or(-1);
                let cdp_timing = response.get("timing").cloned();
                if self.har_recording {
                  if let Some(entry) = self.har_entries.iter_mut().rev().find(|e| e.request_id == request_id) {
                    entry.status = status;
                ...
                if self.request_tracking {
                  ...
                  let resp_mime = response.get("mimeType").and_then(|v| v.as_str()).map(String::from);
            ...
            "Network.loadingFinished" if self.har_recording => {
              ...
              let encoded_data_length = event.params.get("encodedDataLength").and_then(|v| v.as_i64());
              if let Some(entry) = self.har_entries.iter_mut().rev().find(|e| e.request_id == request_id) {
            ...
            "Page.javascriptDialogOpening" => {
              if let Ok(dialog_event) = serde_json::from_value::<JavascriptDialogOpeningEvent>(event.params.clone()) {
                ...
                let auto_handled =
                  self.auto_dialog && matches!(dialog_event.dialog_type.as_str(), "beforeunload" | "alert");
                if !auto_handled {
...
pub async fn execute_command(cmd: &Value, state: &mut DaemonState) -> Value {
  ...
  if let Some(ref mut policy) = state.policy {
    ...
    match policy.check(action) {
      ...
      PolicyResult::Deny(reason) => {
        return error_response(&id, &format!("Action '{}' denied by policy: {}", action, reason));
      }
  ...
  if matches!(state.backend_type, BackendType::WebDriver) && WEBDRIVER_UNSUPPORTED_ACTIONS.contains(&action) {
    return error_response(
      &id,
      &format!("Action '{}' is not supported on the WebDriver backend", action),
    );
  }
  ...
  let result = match action {
    ...
    "state_list" | "state_show" | "state_clear" | "state_clean" | "state_rename" => {
      state::dispatch_state_command(cmd).expect("dispatch_state_command must handle all state_* actions matched here")
    }
  ...
  if let Some(ref server) = state.stream_server {
    ...
    if let Some(ref mgr) = state.browser {
      ...
      if matches!(action, "tab_new" | "tab_switch" | "tab_close" | "open" | "navigate") {
...
async fn load_storage_state_or_rollback(state: &mut DaemonState, path: &Option<String>) -> Result<(), String> {
...
async fn handle_launch(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  let headless = cmd.get("headless").and_then(|v| v.as_bool()).unwrap_or(true);
  let cdp_url = cmd.get("cdpUrl").and_then(|v| v.as_str());
  let cdp_port = cmd.get("cdpPort").and_then(|v| v.as_u64());
  let auto_connect = cmd.get("autoConnect").and_then(|v| v.as_bool()).unwrap_or(false);
  ...
  let launch_options = LaunchOptions {
    ...
    proxy: cmd.get("proxy").and_then(|v| {
      v.as_str()
        .map(|s| s.to_string())
        .or_else(|| v.get("server").and_then(|s| s.as_str()).map(|s| s.to_string()))
    }),
    proxy_bypass: cmd
      .get("proxy")
    ...
    profile: cmd.get("profile").and_then(|v| v.as_str()).map(|s| s.to_string()),
    allow_file_access: cmd.get("allowFileAccess").and_then(|v| v.as_bool()).unwrap_or(false),
    args: cmd
      .get("args")
      .and_then(|v| v.as_array())
    ...
    user_agent: cmd.get("userAgent").and_then(|v| v.as_str()).map(String::from),
    ignore_https_errors: cmd.get("ignoreHTTPSErrors").and_then(|v| v.as_bool()).unwrap_or(false),
    color_scheme: cmd.get("colorScheme").and_then(|v| v.as_str()).map(String::from),
    download_path: cmd.get("downloadPath").and_then(|v| v.as_str()).map(String::from),
    viewport_size: None,
    use_real_keychain: false,
  };
  ...
  if let Some(ref domains) = cmd.get("allowedDomains").and_then(|v| v.as_str()).map(String::from) {
    ...
    if has_domain_filter || has_proxy_auth {
      if let Some(ref mgr) = state.browser {
        if let Ok(session_id) = mgr.active_session_id() {
          if let Some(ref filter) = *df {
            let _ =
              network::install_domain_filter(&mgr.client, session_id, &filter.allowed_domains, has_proxy_auth).await;
            network::sanitize_existing_pages(&mgr.client, &mgr.pages_list(), filter).await;
          } else {
            // No domain filter, but proxy auth needs Fetch.enable
            let _ = network::install_domain_filter_fetch(&mgr.client, session_id, has_proxy_auth).await;
          }
        }
      }
    }
...
async fn launch_ios(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  appium.create_ios_session(Some(&device.name), platform_version).await?;
...
async fn launch_safari(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  let port: u16 = cmd.get("port").and_then(|v| v.as_u64()).map(|p| p as u16).unwrap_or(0);
  ...
  let actual_port = if driver_port > 0 {
    ...
    let listener =
      std::net::TcpListener::bind("127.0.0.1:0").map_err(|e| format!("Failed to find free port: {}", e))?;
    listener
      .local_addr()
...
async fn handle_navigate(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let scoped_headers = cmd.get("headers").and_then(|v| v.as_object()).filter(|m| !m.is_empty());

  if let Some(headers_map) = scoped_headers {
    if let Some(origin) = url::Url::parse(url).ok().map(|u| u.origin().ascii_serialization()) {
...
async fn handle_snapshot(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let options = SnapshotOptions {
    selector: cmd.get("selector").and_then(|v| v.as_str()).map(String::from),
    interactive: cmd.get("interactive").and_then(|v| v.as_bool()).unwrap_or(false),
    compact: cmd.get("compact").and_then(|v| v.as_bool()).unwrap_or(false),
    depth: cmd.get("maxDepth").and_then(|v| v.as_u64()).map(|d| d as usize),
...
async fn handle_screenshot(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let options = ScreenshotOptions {
    ...
    full_page: cmd.get("fullPage").and_then(|v| v.as_bool()).unwrap_or(false),
    format,
    quality: cmd.get("quality").and_then(|v| v.as_i64()).map(|q| q as i32),
    annotate,
    output_dir: cmd.get("screenshotDir").and_then(|v| v.as_str()).map(String::from),
  };
  ...
  if !result.annotations.is_empty() {
    response["annotations"] =
      serde_json::to_value(&result.annotations).map_err(|e| format!("Failed to serialize annotations: {}", e))?;
  }
...
async fn handle_click(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  if new_tab {
    ...
    let call_result = mgr
      .client
      .send_command("Runtime.callFunctionOn", Some(call_params), Some(&effective_session_id))
      .await?;
...
async fn handle_wait(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  if let Some(selector) = cmd.get("selector").and_then(|v| v.as_str()) {
    let state_str = cmd.get("state").and_then(|v| v.as_str()).unwrap_or("visible");
...
async fn handle_reload(state: &mut DaemonState) -> Result<Value, String> {
  ...
  let _ = tokio::time::timeout(tokio::time::Duration::from_secs(10), async {
    loop {
      match rx.recv().await {
        Ok(event) => {
          if event.method == "Page.loadEventFired" && event.session_id.as_deref() == Some(&session_id) {
            return;
          }
        }
...
async fn poll_until_true(
  ...
  loop {
    ...
    if result.result.value.as_ref().and_then(|v| v.as_bool()).unwrap_or(false) {
      return Ok(());
    }
...
async fn handle_diff_snapshot(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let compact = cmd.get("compact").and_then(|v| v.as_bool()).unwrap_or(false);
  let max_depth = cmd.get("maxDepth").and_then(|v| v.as_u64()).map(|d| d as usize);
  let selector = cmd.get("selector").and_then(|v| v.as_str()).map(String::from);
...
async fn handle_credentials_set(cmd: &Value) -> Result<Value, String> {
  let name = cmd.get("name").and_then(|v| v.as_str()).ok_or("Missing 'name'")?;
  let username = cmd
    .get("username")
    .and_then(|v| v.as_str())
...
async fn handle_mouse(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  ...
  let event_type = cmd.get("eventType").and_then(|v| v.as_str()).unwrap_or("mouseMoved");
...
async fn handle_keyboard(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  ...
  match cmd.get("subaction").and_then(|v| v.as_str()) {
    ...
    Some("insertText") => {
      ...
      mgr
        .client
        .send_command("Input.insertText", Some(json!({ "text": text })), Some(&session_id))
        .await?;
  ...
  let event_type = cmd.get("eventType").and_then(|v| v.as_str()).unwrap_or("keyDown");
  let key = cmd.get("key").and_then(|v| v.as_str());
...
async fn handle_tab_switch(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  if let Some(ref server) = state.stream_server {
    if let Ok(dims) = mgr
      .evaluate("JSON.stringify([window.innerWidth,window.innerHeight])", None)
      .await
    {
...
async fn handle_viewport(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let scale = cmd.get("deviceScaleFactor").and_then(|v| v.as_f64()).unwrap_or(1.0);
...
async fn handle_download(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let dest = download_dir.join(raw_dest.file_name().ok_or("Invalid download path: no filename")?);
  ...
  loop {
    ...
    match tokio::time::timeout(remaining, rx.recv()).await {
      Ok(Ok(event)) => {
        ...
        if is_download_event(&event.method, "Browser.downloadWillBegin", "Page.downloadWillBegin") {
        ...
        if is_download_event(&event.method, "Browser.downloadProgress", "Page.downloadProgress") {
  ...
  if let Some(guid) = downloaded_guid {
    ...
    if guid_path.exists() {
      std::fs::rename(&guid_path, &dest).map_err(|e| format!("Failed to rename downloaded file: {}", e))?;
    } else {
      // The file might have been saved under its original name instead
      // of the GUID (e.g. when Chrome falls back to "allow" behavior).
      if !dest.exists() {
        return Err(format!("Downloaded file not found at expected path (GUID: {})", guid));
      }
    }
  } else {
    ...
    if !dest.exists() {
      return Err("Download completed but could not determine the downloaded file name".to_string());
    }
  }
...
async fn handle_profiler_start(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let categories = cmd
    ...
    .and_then(|v| v.as_array())
    .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect());
  native_tracing::profiler_start(&mgr.client, &session_id, &mut state.tracing_state, categories).await
}
...
async fn handle_recording_start(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let recording_url = cmd.get("url").and_then(|v| v.as_str()).filter(|s| !s.is_empty());
  ...
  let (client, new_session_id) = {
    ...
    let nav_url = if let Some(u) = recording_url {
      ...
      mgr.get_url().await.unwrap_or_else(|_| "about:blank".to_string())
    };
    ...
    if nav_url != "about:blank" {
      let _ = mgr
        .client
        .send_command("Page.navigate", Some(json!({ "url": nav_url })), Some(&new_session_id))
        .await;
      tokio::time::sleep(tokio::time::Duration::from_millis(1000)).await;
    }
  ...
  let save_path = match path {
    ...
    None => {
      ...
      let timestamp = std::time::SystemTime::now()
        ...
        .as_millis();
      dir
        .join(format!("page-{}.pdf", timestamp))
        .to_string_lossy()
        .to_string()
    }
  };
...
async fn handle_tap(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  interaction::tap_touch(&mgr.client, &session_id, &state.ref_map, sel, &state.iframe_sessions).await?;
  Ok(json!({ "tapped": sel }))
}
...
async fn handle_upload(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  ...
  let files: Vec<String> = cmd
    ...
    .and_then(|v| v.as_array())
    .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect())
    .or_else(|| cmd.get("file").and_then(|v| v.as_str()).map(|s| vec![s.to_string()]))
    .unwrap_or_default();
...
async fn handle_react_tree(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  ...
  let nodes: Vec<react::TreeNode> =
    serde_json::from_value(nodes_json).map_err(|e| format!("Failed to parse tree nodes: {}", e))?;
...
async fn handle_react_renders_start(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  ...
  let identifier = mgr.add_script_to_evaluate(react::scripts::RENDERS_INIT).await?;
...
async fn handle_react_renders_stop(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  ...
  let data: react::RendersData =
    serde_json::from_value(data_json.clone()).map_err(|e| format!("Failed to parse renders data: {}", e))?;
...
async fn handle_react_suspense(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  ...
  let return_json = cmd.get("json").and_then(|v| v.as_bool()).unwrap_or(false);
  let only_dynamic = cmd.get("onlyDynamic").and_then(|v| v.as_bool()).unwrap_or(false);
  if return_json {
    ...
    if only_dynamic {
      let filtered: Vec<&react::Boundary> = boundaries
        .iter()
        .filter(|b| {
          b.parent_id != 0 && (b.is_suspended || !b.suspended_by.is_empty() || b.unknown_suspenders.is_some())
        })
        .collect();
...
async fn handle_vitals(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  {
    let mgr = state.browser.as_ref().ok_or("Browser not launched")?;
    let _ = mgr.add_script_to_evaluate(react::scripts::VITALS_INIT).await;
  }
  ...
  if return_json {
    ...
    let data: react::VitalsData =
      serde_json::from_value(data_value.clone()).map_err(|e| format!("Failed to parse vitals data: {}", e))?;
...
async fn handle_addstyle(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  let mgr = state.browser.as_ref().ok_or("Browser not launched")?;
  let content = cmd.get("content").or_else(|| cmd.get("css")).and_then(|v| v.as_str());
  let url = cmd.get("url").and_then(|v| v.as_str());
...
async fn handle_clipboard(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  ...
  match action {
    ...
    "copy" => {
      interaction::press_key_with_modifiers(&mgr.client, &session_id, "c", Some(modifier)).await?;
      Ok(json!({ "copied": true }))
    }
    "paste" => {
      interaction::press_key_with_modifiers(&mgr.client, &session_id, "v", Some(modifier)).await?;
      Ok(json!({ "pasted": true }))
    }
    _ => {
...
fn write_stream_file(session_id: &str, port: u16) -> Result<(), String> {
  ...
  fs::write(&path, port.to_string()).map_err(|e| format!("Failed to write stream metadata '{}': {}", path.display(), e))
}
...
async fn handle_stream_enable(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let requested_port = match cmd.get("port").and_then(|value| value.as_u64()) {
    Some(raw) => u16::try_from(raw).map_err(|_| format!("Invalid stream port '{}': expected 0-65535", raw))?,
    None => 0,
  };
...
async fn handle_screencast_start(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let max_width = cmd.get("maxWidth").and_then(|v| v.as_i64()).unwrap_or(default_w as i64) as i32;
  let max_height = cmd
    .get("maxHeight")
    .and_then(|v| v.as_i64())
    .unwrap_or(default_h as i64) as i32;

  stream::start_screencast(&mgr.client, &session_id, format, quality, max_width, max_height).await?;
  state.screencasting = true;

  if let Some(ref server) = state.stream_server {
    server.set_screencasting(true).await;
    server
      .broadcast_status(true, true, max_width as u32, max_height as u32, &state.engine)
      .await;
  }
...
async fn handle_screencast_stop(state: &mut DaemonState) -> Result<Value, String> {
  ...
  if let Some(ref server) = state.stream_server {
    ...
    let (vw, vh) = server.viewport().await;
    server.broadcast_status(true, false, vw, vh, &state.engine).await;
  }
...
async fn execute_subaction(cmd: &Value, state: &mut DaemonState, selector: &str) -> Result<Value, String> {
  let subaction = cmd.get("subaction").and_then(|v| v.as_str()).unwrap_or("click");
  let mgr = state.browser.as_ref().ok_or("Browser not launched")?;
  let session_id = mgr.active_session_id()?.to_string();
...
async fn handle_nth(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  if !result.result.value.as_ref().and_then(|v| v.as_bool()).unwrap_or(false) {
    return Err(format!("No element at index {} for selector '{}'", index, selector));
  }
...
async fn handle_drag(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let (sx, sy, source_session_id) =
    super::element::resolve_element_center(&mgr.client, &session_id, &state.ref_map, source, &state.iframe_sessions)
      .await?;
  let (tx, ty, target_session_id) =
    super::element::resolve_element_center(&mgr.client, &session_id, &state.ref_map, target, &state.iframe_sessions)
      .await?;
...
async fn handle_expose(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  ...
  mgr
    .client
    .send_command("Runtime.addBinding", Some(json!({ "name": name })), Some(&session_id))
    .await?;
...
async fn handle_responsebody(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  ...
  loop {
    let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
    if remaining.is_zero() {
      return Err(format!("Timeout waiting for response matching '{}'", url_pattern));
    }
  ...
  loop {
    ...
    match tokio::time::timeout(remaining, rx.recv()).await {
      Ok(Ok(event)) => {
        ...
        let is_page_session = event.session_id.as_deref() == Some(&session_id);
        let is_progress =
          event.method == "Browser.downloadProgress" || (event.method == "Page.downloadProgress" && is_page_session);

        if is_progress && event.params.get("state").and_then(|v| v.as_str()) == Some("completed") {
          let path = cmd.get("path").and_then(|v| v.as_str()).unwrap_or("download");
...
async fn handle_window_new(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  if let Some(viewport) = cmd.get("viewport") {
    let width = viewport.get("width").and_then(|v| v.as_i64()).unwrap_or(1280) as i32;
    let height = viewport.get("height").and_then(|v| v.as_i64()).unwrap_or(720) as i32;
...
async fn handle_diff_screenshot(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  ...
  let current_bytes = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &result.base64)
    .map_err(|e| format!("Failed to decode screenshot: {}", e))?;

  let baseline_bytes = std::fs::read(baseline_path).map_err(|e| format!("Failed to read baseline: {}", e))?;
  ...
  if let (Some(out_path), Some(ref diff_data)) = (output_path, &result.diff_image) {
    std::fs::write(out_path, diff_data).map_err(|e| format!("Failed to write diff image: {}", e))?;
  }
...
async fn handle_har_stop(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let har_str = serde_json::to_string_pretty(&har).map_err(|e| format!("Failed to serialize HAR: {}", e))?;
...
fn har_entry_to_json(e: HarEntry) -> Value {
  ...
  let (timings, total_time) = har_compute_timings(e.cdp_timing.as_ref(), e.loading_finished_timestamp);
...
fn har_compute_timings(cdp_timing: Option<&Value>, loading_finished_ts: Option<f64>) -> (Value, f64) {
  ...
  for route in routes {
    ...
    if matches {
      ...
      if let Some(ref resp) = route.response {
        ...
        let encoded = base64::Engine::encode(&base64::engine::general_purpose::STANDARD, body_str.as_bytes());
        let mut headers = vec![];
...
async fn build_fetch_patterns(state: &DaemonState) -> Vec<Value> {
  ...
  let mut patterns: Vec<Value> = routes.iter().map(|r| json!({ "urlPattern": r.url_pattern })).collect();
  ...
  let has_proxy_creds = state.proxy_credentials.read().await.is_some();
  if (has_domain_filter || has_origin_headers || has_proxy_creds) && !patterns.iter().any(|p| p["urlPattern"] == "*") {
...
async fn handle_route(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let response = cmd.get("response").and_then(|v| {
    ...
    Some(RouteResponse {
      ...
      content_type: v.get("contentType").and_then(|s| s.as_str()).map(String::from),
...
async fn wait_for_any_selector(
  ...
  loop {
    ...
    tokio::time::sleep(tokio::time::Duration::from_millis(AUTH_LOGIN_SELECTOR_POLL_INTERVAL_MS)).await;
  }
}

async fn handle_auth_save(cmd: &Value) -> Result<Value, String> {
  let name = cmd.get("name").and_then(|v| v.as_str()).ok_or("Missing 'name'")?;
  let url = cmd.get("url").and_then(|v| v.as_str()).ok_or("Missing 'url'")?;
  let username = cmd
    .get("username")
    .and_then(|v| v.as_str())
    .ok_or("Missing 'username'")?;
...
async fn handle_auth_login(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  ...
  let auto_submit_selectors = ["button[type=submit]", "input[type=submit]", "button:not([type])"];
  ...
  let user_sel = if let Some(s) = username_sel {
    ...
    match wait_for_any_selector(&mgr.client, &session_id, &preferred_user_selectors, preferred_window_ms).await {
  ...
  wait_for_selector(&mgr.client, &session_id, &pass_sel, "visible", auth_timeout_ms)
    .await
  ...
  let sub_sel = if let Some(s) = submit_sel {
    ...
    wait_for_any_selector(&mgr.client, &session_id, &auto_submit_selectors, auth_timeout_ms)
      .await
      .map_err(|_| {
        format!(
...
async fn handle_confirm(_cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  let pending = state.pending_confirmation.take().ok_or("No pending confirmation")?;
...
fn build_mouse_event_params(
  ...
  DispatchMouseEventParams {
    ...
    x,
    y,
    button: Some(button.unwrap_or(primary_button_from_mask(next_buttons)).to_string()),
...
async fn handle_input_mouse(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
  let mgr = state.browser.as_ref().ok_or("Browser not launched")?;
  let session_id = mgr.active_session_id()?.to_string();
  let event_type = cmd.get("type").and_then(|v| v.as_str()).unwrap_or("mouseMoved");
  let params = build_mouse_event_params(
    ...
    cmd.get("buttons").and_then(|v| v.as_i64()).map(|v| v as i32),
    cmd.get("clickCount").and_then(|v| v.as_i64()).map(|v| v as i32),
    cmd.get("deltaX").and_then(|v| v.as_f64()),
    cmd.get("deltaY").and_then(|v| v.as_f64()),
    cmd.get("modifiers").and_then(|v| v.as_i64()).map(|v| v as i32),
  );
...
async fn handle_input_keyboard(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  let mgr = state.browser.as_ref().ok_or("Browser not launched")?;
  let session_id = mgr.active_session_id()?.to_string();
  let event_type = cmd.get("type").and_then(|v| v.as_str()).unwrap_or("keyDown");
...
async fn handle_input_touch(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
  let mgr = state.browser.as_ref().ok_or("Browser not launched")?;
  let session_id = mgr.active_session_id()?.to_string();
  let event_type = cmd.get("type").and_then(|v| v.as_str()).unwrap_or("touchStart");
...
mod tests {
  ...
  fn unique_socket_dir(label: &str) -> PathBuf {
    let nanos = std::time::SystemTime::now()
      ...
      .as_nanos();
    std::env::temp_dir().join(format!("agent-browser-{label}-{}-{nanos}", std::process::id()))
  }
  ...
  async fn test_stream_enable_port_conflict_returns_error() {
    ...
    let listener = std::net::TcpListener::bind("127.0.0.1:0").expect("test should reserve an ephemeral port");
    let port = listener.local_addr().expect("listener should have local addr").port();
  ...
  fn test_har_set_cookie_strips_attributes_before_equal_split() {
    let entry = HarEntry {
      ...
      http_version: "HTTP/1.1".to_string(),
      response_headers: vec![("set-cookie".to_string(), "token=abc; Path=/; HttpOnly".to_string())],
  ...
  async fn test_execute_har_stop_skips_browser_auto_launch() {
    let path = std::env::temp_dir().join(format!("agent-browser-har-stop-{}.har", unix_timestamp_millis()));
    let mut state = DaemonState::new();
    state.har_entries.push(HarEntry {
