...
pub async fn trace_stop(
  ...
  client.send_command_no_params("Tracing.end", Some(session_id)).await?;
      ...
      Ok(Ok(event)) => {
        ...
        match event.method.as_str() {
          ...
          "Tracing.tracingComplete" => {
            stream_handle = event.params.get("stream").and_then(|v| v.as_str()).map(String::from);
            break;
          }
          _ => {}
        }
  ...
  if let Some(handle) = stream_handle {
    ...
    // Close the IO stream
    let _ = client
      .send_command("IO.close", Some(json!({ "handle": handle })), Some(session_id))
      .await;
  }
  ...
  let save_path = match path {
    ...
    None => {
      ...
      let timestamp = std::time::SystemTime::now()
        ...
        .as_millis();
      dir
        .join(format!("trace-{}.json", timestamp))
        .to_string_lossy()
        .to_string()
    }
  };
  ...
  let json_str = serde_json::to_string(&trace_json).map_err(|e| format!("Failed to serialize trace: {}", e))?;
  std::fs::write(&save_path, json_str).map_err(|e| format!("Failed to write trace to {}: {}", save_path, e))?;
...
pub async fn profiler_start(
  ...
  let cats: Vec<String> =
    categories.unwrap_or_else(|| DEFAULT_PROFILER_CATEGORIES.iter().map(|s| s.to_string()).collect());
...
pub async fn profiler_stop(
  ...
  let save_path = match path {
    ...
    None => {
      ...
      let timestamp = std::time::SystemTime::now()
        ...
        .as_millis();
      dir
        .join(format!("profile-{}.json", timestamp))
        .to_string_lossy()
        .to_string()
    }
  };
  ...
  let json_str = serde_json::to_string(&profile).map_err(|e| format!("Failed to serialize profile: {}", e))?;
  std::fs::write(&save_path, json_str).map_err(|e| format!("Failed to write profile to {}: {}", save_path, e))?;
  ...
  if dropped {
    result.as_object_mut().unwrap().insert(
      "warning".to_string(),
      Value::String(format!("Events exceeded {} limit; some dropped", MAX_PROFILE_EVENTS)),
    );
  }
...
async fn read_io_stream(client: &CdpClient, session_id: &str, handle: &str) -> Result<String, String> {
