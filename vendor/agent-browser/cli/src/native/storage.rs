...
pub async fn storage_get(
  ...
  if let Some(k) = key {
    let js = format!("{}.getItem({})", st, serde_json::to_string(k).unwrap_or_default());
...
pub async fn storage_clear(client: &CdpClient, session_id: &str, storage_type: &str) -> Result<(), String> {
  let st = storage_js_name(storage_type);
