...
fn build_insight(b: &Boundary) -> BoundaryInsight {
  ...
  let mut blockers: Vec<ActionableBlocker> = b.suspended_by.iter().map(build_actionable_blocker).collect();
  blockers.sort_by(|a, b| {
    b.actionability
      .cmp(&a.actionability)
      .then_with(|| b.kind.weight().cmp(&a.kind.weight()).then_with(|| a.name.cmp(&b.name)))
  });
  let primary = blockers.first().cloned();
  let recommendation = recommend_fix(boundary_kind, primary.as_ref(), b.unknown_suspenders.as_deref());
...
fn infer_boundary_kind(b: &Boundary) -> BoundaryKind {
  ...
  if name_ends_slash || owner_names.contains(&"LoadingBoundary") || owner_names.contains(&"OuterLayoutRouter") {
...
fn classify_blocker(s: &Suspender, source_frame: Option<&StackFrame>) -> BlockerKind {
  ...
  match name.as_str() {
    ...
    "cookies" | "headers" | "connection" | "params" | "searchparams" | "draftmode" => return BlockerKind::RequestApi,
    _ => {}
  }
  if name == "rsc stream" {
...
fn recommend_fix(
  ...
  if boundary_kind == BoundaryKind::RouteSegment && primary.is_some_and(|p| p.kind == BlockerKind::ClientHook) {
  ...
  if let Some(p) = primary {
    match p.kind {
      ...
      BlockerKind::RequestApi | BlockerKind::ServerFetch => {
        return "Push the request-bound async work into a smaller leaf or split static siblings out of this boundary."
          .to_string();
      }
      BlockerKind::Cache => {
        return "Check whether caching or runtime prefetch can move this personalized content into the shell."
          .to_string();
      }
      BlockerKind::Stream => {
        return "Keep the stream behind Suspense, but extract any static shell content outside the boundary."
          .to_string();
      }
  ...
  "No primary blocker was identified. Inspect the boundary source and owner chain directly.".to_string()
}
...
fn build_root_causes(holes: &[BoundaryInsight]) -> Vec<RootCauseGroup> {
  ...
  for hole in holes {
    ...
    let display_name = hole.name.clone().unwrap_or_else(|| format!("boundary-{}", hole.id));
    groups
...
fn format_report(report: &AnalysisReport, only_dynamic: bool) -> String {
  ...
  if !report.holes.is_empty() {
    ...
    lines.push("| Boundary | Type | Primary blocker | Source | Suggested next step |".to_string());
