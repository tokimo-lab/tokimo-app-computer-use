...
pub fn format_vitals_report(d: &VitalsData) -> String {
  ...
  match &d.lcp {
    Some(lcp) => {
      ...
      lines.push(format!("  LCP {:>10}{}", format!("{}ms", lcp.start_time), label));
    }
  ...
  match &d.hydration {
    ...
    None => lines.push("## React Hydration - no data (requires React profiling build)".to_string()),
  }
  ...
  if !d.hydrated_components.is_empty() {
    ...
    for c in d.hydrated_components.iter().take(30) {
      lines.push(format!("  {:<40} {:>10}", c.name, format!("{}ms", c.duration)));
    }
    if d.hydrated_components.len() > 30 {
      lines.push(format!("  ... and {} more", d.hydrated_components.len() - 30));
    }
  }
