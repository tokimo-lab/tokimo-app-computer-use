...
pub(super) async fn handle_models_request(stream: &mut tokio::net::TcpStream, origin: Option<&str>) {
  ...
  messages
    ...
    .map(|m| {
      let content_len = m
        ...
        .unwrap_or(0);
      let tc_len = m.get("tool_calls").map(|t| t.to_string().len()).unwrap_or(0);
      content_len + tc_len
    })
    .sum()
}
...
fn compress_image_to_jpeg(raw_bytes: &[u8]) -> Option<Vec<u8>> {
  ...
  let img = if img.width() > SCREENSHOT_MAX_WIDTH {
    img.resize(SCREENSHOT_MAX_WIDTH, u32::MAX, image::imageops::FilterType::Triangle)
  } else {
    img
  };
  ...
  for line in text.lines() {
    ...
    for suffix in [".png", ".jpg", ".jpeg"] {
      if let Some(pos) = trimmed.to_lowercase().rfind(suffix) {
        ...
        let start = candidate.rfind(|c: char| c.is_whitespace()).map(|i| i + 1).unwrap_or(0);
...
pub(super) async fn handle_chat_request(stream: &mut tokio::net::TcpStream, body: &str, origin: Option<&str>) {
  let cors = cors_headers_for_origin(origin);
  let gateway_url = std::env::var("AI_GATEWAY_URL")
    .unwrap_or_else(|_| DEFAULT_AI_GATEWAY_URL.to_string())
    .trim_end_matches('/')
  ...
  let api_key = match std::env::var("AI_GATEWAY_API_KEY") {
    ...
    Err(_) => {
      let err =
        r#"{"error":"AI_GATEWAY_API_KEY not set. Set the AI_GATEWAY_API_KEY environment variable to enable AI chat."}"#;
      let resp = format!(
  ...
  let default_model = std::env::var("AI_GATEWAY_MODEL").unwrap_or_else(|_| "anthropic/claude-sonnet-4.6".to_string());
  ...
  for _step in 0..50 {
    ...
    for (tc_id, tc_name, tc_args) in &tool_calls {
      ...
      let result = match tokio::time::timeout(TOOL_TIMEOUT, execute_chat_tool(&session, command)).await {
        Ok(r) => r,
