...
async fn write_http_response(stream: &mut tokio::net::TcpStream, status: &str, content_type: &str, body: &[u8]) {
...
async fn write_json_error_response_no_cors(stream: &mut tokio::net::TcpStream, status: &'static str, error: &str) {
  let body = build_json_error_body(error);
  write_http_response_no_cors(stream, status, "application/json; charset=utf-8", body.as_bytes()).await;
}
...
fn header_matches_host(request: &str, header_name: &str) -> Option<bool> {
  let authority = request_header_value(request, header_name).and_then(normalize_origin_authority)?;
...
fn sessions_json_has_active_port(sessions_json: &str, port: u16) -> Result<bool, String> {
  let sessions: Vec<Value> =
    serde_json::from_str(sessions_json).map_err(|e| format!("Failed to parse active sessions: {e}"))?;
...
fn require_active_session_port(port: u16) -> Result<(), DashboardProxyError> {
  ...
  let is_active = sessions_json_has_active_port(&sessions_json, port).map_err(DashboardProxyError::bad_gateway)?;
  if is_active {
    Ok(())
  } else {
...
fn parse_upstream_http_response(response: &[u8]) -> Result<(String, String, Vec<u8>), String> {
  ...
  let header_str =
    std::str::from_utf8(header_bytes).map_err(|e| format!("Upstream response headers were not valid UTF-8: {e}"))?;
...
async fn proxy_session_http_route(
  ...
  let request = format!("GET {upstream_path} HTTP/1.1\r\nHost: 127.0.0.1:{port}\r\nConnection: close\r\n\r\n");

  tokio::time::timeout(PROXY_TIMEOUT, async {
    let mut upstream = tokio::net::TcpStream::connect(("127.0.0.1", port))
      .await
      .map_err(|e| DashboardProxyError::bad_gateway(format!("Failed to connect to session {port}: {e}")))?;
    upstream
      .write_all(request.as_bytes())
      .await
      .map_err(|e| DashboardProxyError::bad_gateway(format!("Failed to proxy request to session {port}: {e}")))?;
    ...
    (&mut upstream)
      ...
      .await
      .map_err(|e| DashboardProxyError::bad_gateway(format!("Failed to read session {port} response: {e}")))?;
    ...
    match endpoint {
      SessionProxyEndpoint::Stream => {
        ...
        if !is_same_origin_ws_request(&peeked_request) {
          write_json_error_response_no_cors(&mut stream, "403 Forbidden", "Origin does not match Host header.").await;
          return;
        }
        if let Err(error) = require_active_session_port(port) {
          write_json_error_response_no_cors(&mut stream, error.status, &error.message).await;
          return;
        }
...
async fn kill_session(body: &str) -> Result<String, String> {
  ...
  let pid_str = std::fs::read_to_string(&pid_path).map_err(|_| format!("No PID file for session '{}'", session))?;
  let pid: u32 = pid_str
    .trim()
    .parse()
...
pub(super) async fn spawn_session(body: &str) -> Result<String, String> {
  ...
  let mut cmd = tokio::process::Command::new(&exe);
  cmd.arg("open").arg("about:blank").arg("--session").arg(session);
