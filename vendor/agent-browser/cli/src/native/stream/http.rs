...
fn normalize_origin_authority(origin: &str) -> Option<String> {
  ...
  let default_port =
    (url.scheme() == "http" && url.port() == Some(80)) || (url.scheme() == "https" && url.port() == Some(443));
  Some(match url.port() {
...
fn is_loopback_authority(authority: &str) -> bool {
  matches!(authority_host(authority), "localhost" | "127.0.0.1" | "::1" | "[::1]")
}

fn header_authority_matches_host(request: &str, header_name: &str) -> bool {
  let Some(authority) = request_header_value(request, header_name).and_then(normalize_origin_authority) else {
    return false;
  };
...
async fn write_json_error_response_no_cors(stream: &mut tokio::net::TcpStream, status: &str, error: &str) {
  let body = format!(
...
pub(super) async fn handle_http_request(
  ...
  if method == "POST" {
    ...
    if full_body.is_none() && (path == "/api/chat" || path == "/api/sessions" || path == "/api/command") {
...
mod tests {
  ...
  #[cfg(unix)]
  async fn spawn_fake_daemon(socket_dir: &std::path::Path, session_name: &str) -> oneshot::Receiver<String> {
  ...
  async fn cross_origin_command_post_is_rejected_without_relaying_to_daemon() {
    ...
    let socket_dir = tempfile::Builder::new().prefix("ab-").tempdir_in(temp_parent).unwrap();
    let guard = EnvGuard::new(&["AGENT_BROWSER_SOCKET_DIR", "XDG_RUNTIME_DIR"]);
    guard.set("AGENT_BROWSER_SOCKET_DIR", socket_dir.path().to_str().unwrap());
