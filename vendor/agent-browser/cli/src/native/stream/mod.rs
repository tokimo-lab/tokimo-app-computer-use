...
impl StreamServer {
  pub async fn start(preferred_port: u16, client: Arc<CdpClient>, session_id: String) -> Result<Self, String> {
  ...
  async fn start_inner(
    ...
    let listener = match TcpListener::bind(&addr).await {
      Ok(l) => l,
      Err(_) if allow_port_fallback && preferred_port != 0 => TcpListener::bind("127.0.0.1:0")
        .await
  ...
  pub fn broadcast_result(&self, id: &str, action: &str, success: bool, data: &Value, duration_ms: u64) {
    let msg = json!({
        "type": "result",
        "id": id,
        "action": action,
