...
pub async fn click(
  ...
  let (x, y, effective_session_id) =
    resolve_element_center(client, session_id, ref_map, selector_or_ref, iframe_sessions).await?;
...
pub async fn dblclick(
  ...
  click(client, session_id, ref_map, selector_or_ref, "left", 2, iframe_sessions).await
}
  ...
  for ch in text.chars() {
    if matches!(ch, '\n' | '\r' | '\t') {
      ...
      client
        .send_command_typed::<_, Value>(
          "Input.insertText",
          &InsertTextParams { text: ch.to_string() },
          Some(session_id),
        )
        .await?;
    }
...
pub async fn check(
  ...
  let is_checked =
    super::element::is_element_checked(client, session_id, ref_map, selector_or_ref, iframe_sessions).await?;
  if !is_checked {
    click(client, session_id, ref_map, selector_or_ref, "left", 1, iframe_sessions).await?;
    ...
    if !super::element::is_element_checked(client, session_id, ref_map, selector_or_ref, iframe_sessions).await? {
      js_click_checkbox(client, session_id, ref_map, selector_or_ref, iframe_sessions).await?;
    }
  }
  Ok(())
}
...
pub async fn scroll_into_view(
  ...
  client
    .send_command_typed::<_, Value>(
      "Runtime.callFunctionOn",
      &CallFunctionOnParams {
        function_declaration: "function() { this.scrollIntoView({ block: 'center', inline: 'center' }); }".to_string(),
        object_id: Some(object_id),
        arguments: None,
        return_by_value: Some(true),
        await_promise: Some(false),
...
mod tests {
  ...
  fn test_unmapped_chars_return_zero_keycode() {
    for ch in ['@', '#', '$', '%', '^', '&', '*', '(', ')', '€', '£', '你'] {
      ...
      assert_eq!(vk, 0, "char {:?}: unmapped char should have VK 0, got {}", ch, vk);
