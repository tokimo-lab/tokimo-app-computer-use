...
pub async fn take_screenshot(
  ...
  let target_rect = if options.annotate {
    match options.selector.as_deref() {
      Some(selector) => get_rect_for_selector(client, session_id, ref_map, selector, iframe_sessions).await?,
      None => None,
    }
  } else {
    None
  ...
  let base64 = capture_screenshot_base64(client, session_id, ref_map, options, iframe_sessions).await;
  ...
  let ext = if options.format == "jpeg" { "jpg" } else { "png" };
...
async fn capture_screenshot_base64(
  ...
  if options.full_page {
    ...
    let content_size = metrics.get("contentSize").or_else(|| metrics.get("cssContentSize"));
    ...
    if let Some(rect) = get_rect_for_selector(client, session_id, ref_map, selector, iframe_sessions).await? {
      params.clip = Some(Viewport {
        x: rect.x,
        y: rect.y,
        width: rect.width,
...
async fn collect_annotations(
  ...
  let with_backend_ids: Vec<(String, super::element::RefEntry, i64)> = entries
    .iter()
    .filter_map(|(ref_id, entry)| entry.backend_node_id.map(|bid| (ref_id.clone(), entry.clone(), bid)))
    .collect();
...
async fn get_rect_for_selector(
  ...
  let (object_id, effective_session_id) =
    super::element::resolve_element_object_id(client, session_id, ref_map, selector, iframe_sessions).await?;
  get_rect_for_object(client, &effective_session_id, &object_id).await
}

async fn get_rect_for_object(client: &CdpClient, session_id: &str, object_id: &str) -> Result<Option<Rect>, String> {
...
fn filter_annotations(annotations: Vec<RawAnnotation>, target_rect: Option<&Rect>) -> Vec<RawAnnotation> {
...
async fn inject_annotation_overlay(
  ...
  let expression = format!(
    ...
    overlay_id = serde_json::to_string(ANNOTATION_OVERLAY_ID).unwrap_or_else(|_| "\"\"".to_string()),
  );
...
fn save_screenshot(
  ...
  std::fs::write(&save_path, &bytes).map_err(|e| format!("Failed to save screenshot to {}: {}", save_path, e))?;
...
fn get_screenshot_dir() -> PathBuf {
  if let Some(home) = dirs::home_dir() {
    ...
    std::env::temp_dir().join("agent-browser").join("screenshots")
  }
}
