...
fn build_chrome_args(options: &LaunchOptions) -> Result<ChromeArgs, String> {
  ...
  let has_extensions = options.extensions.as_ref().is_some_and(|exts| !exts.is_empty());
  ...
  let (user_data_dir, temp_user_data_dir) = if let Some(ref profile) = options.profile {
    ...
    let dir = std::env::temp_dir().join(format!("agent-browser-chrome-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).map_err(|e| format!("Failed to create temp profile dir: {}", e))?;
    ...
    Err(primary_err) => {
      ...
      match wait_for_ws_url_until(reader, deadline) {
        ...
        Err(fallback_err) => {
          let _ = child.kill();
          cleanup_temp_dir(&temp_user_data_dir);
          return Err(format!("{}\n(also tried parsing stderr) {}", primary_err, fallback_err));
        }
      }
    }
  };
...
pub fn find_chrome() -> Option<PathBuf> {
  ...
  #[cfg(target_os = "windows")]
  {
    ...
    if let Ok(local) = std::env::var("LOCALAPPDATA") {
      ...
      let brave = PathBuf::from(&local).join(r"BraveSoftware\Brave-Browser\Application\brave.exe");
      if brave.exists() {
        return Some(brave);
      }
    }
...
pub fn read_devtools_active_port(user_data_dir: &Path) -> Option<(u16, String)> {
  ...
  let ws_path = lines.next().unwrap_or("/devtools/browser").trim().to_string();
  Some((port, ws_path))
}
...
pub fn resolve_chrome_profile(user_data_dir: &Path, input: &str) -> Result<String, String> {
  ...
  if let Some(p) = profiles.iter().find(|p| p.directory.to_lowercase() == input_lower) {
    return Ok(p.directory.clone());
  }
...
pub fn copy_chrome_profile(user_data_dir: &Path, profile_directory: &str) -> Result<PathBuf, String> {
  let temp_dir = std::env::temp_dir().join(format!("agent-browser-profile-{}", uuid::Uuid::new_v4()));
  std::fs::create_dir_all(&temp_dir).map_err(|e| format!("Failed to create temp profile dir: {}", e))?;
  ...
  if !src_profile.is_dir() {
    ...
    return Err(format!("Profile directory not found: {}", src_profile.display()));
  }
...
fn build_puppeteer_binary_path(version_dir: &Path) -> PathBuf {
  ...
  let arm = version_dir.join("chrome-mac-arm64/Google Chrome for Testing.app/Contents/MacOS/Google Chrome for Testing");
  if arm.exists() {
    return arm;
  }
  version_dir.join("chrome-mac-x64/Google Chrome for Testing.app/Contents/MacOS/Google Chrome for Testing")
}
...
mod tests {
  ...
  fn test_build_args_headless_includes_headless_flag() {
    ...
    assert!(result.args.iter().any(|a| a == "--enable-unsafe-swiftshader"));
  ...
  fn test_build_args_temp_user_data_dir_created() {
    ...
    assert!(dir.exists());
    assert!(result.args.iter().any(|a| a.starts_with("--user-data-dir=")));
    let _ = std::fs::remove_dir_all(dir);
  }
  ...
  fn test_build_args_profile_no_temp_dir() {
    ...
    assert!(result.args.iter().any(|a| a == "--user-data-dir=/tmp/my-profile"));
  }
  ...
  fn test_build_args_headless_with_extensions_skips_headless_flag() {
    ...
    assert!(result.args.iter().any(|a| a.starts_with("--load-extension=")));
    if let Some(ref dir) = result.temp_user_data_dir {
      let _ = std::fs::remove_dir_all(dir);
    }
  }
  ...
  fn test_build_args_ignore_https_errors_includes_flag() {
    ...
    let result = build_chrome_args(&opts).unwrap();
    assert!(result.args.iter().any(|a| a == "--ignore-certificate-errors"));
    if let Some(ref dir) = result.temp_user_data_dir {
      let _ = std::fs::remove_dir_all(dir);
    }
  }
  ...
  fn test_chrome_process_drop_cleans_temp_dir() {
    let dir = std::env::temp_dir().join(format!("agent-browser-chrome-drop-test-{}", uuid::Uuid::new_v4()));
  ...
  fn create_fake_local_state(base: &Path, profiles: &[(&str, &str)]) {
    ...
    for (dir_name, display_name) in profiles {
      ...
      entry.insert("name".to_string(), serde_json::Value::String(display_name.to_string()));
  ...
  fn create_fake_profile(user_data_dir: &Path, profile_dir: &str) {
    ...
    std::fs::write(profile_path.join("Local Storage/leveldb/CURRENT"), "fake-leveldb").unwrap();
  ...
  fn test_copy_dir_recursive_excludes() {
    ...
    for excluded in PROFILE_COPY_EXCLUDE_DIRS {
      assert!(!dst.join(excluded).exists(), "{} should be excluded", excluded);
    }
  }
