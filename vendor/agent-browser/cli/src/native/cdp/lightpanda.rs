...
pub fn find_lightpanda() -> Option<PathBuf> {
  ...
  if let Some(home) = dirs::home_dir() {
    let candidates = [home.join(".lightpanda/lightpanda"), home.join(".local/bin/lightpanda")];
    for c in &candidates {
      if c.exists() {
        return Some(c.clone());
      }
...
pub async fn launch_lightpanda(options: &LightpandaLaunchOptions) -> Result<LightpandaProcess, String> {
  ...
  let ws_url = match wait_for_lightpanda_ready(&mut child, port, &log_buffer, LIGHTPANDA_STARTUP_TIMEOUT).await {
    Ok(url) => url,
    Err(e) => {
      let _ = child.kill();
      let _ = child.wait();
...
fn start_log_drainers(child: &mut Child) -> Result<(LaunchLogBuffer, Vec<std::thread::JoinHandle<()>>), String> {
  ...
  let stdout_handle = std::thread::spawn(move || drain_reader(stdout, move |line| stdout_logs.push_stdout(line)));
  let stderr_handle = std::thread::spawn(move || drain_reader(stderr, move |line| stderr_logs.push_stderr(line)));
...
async fn wait_for_lightpanda_ready(
  ...
  loop {
    ...
    match discover_cdp_url_with_timeout("127.0.0.1", port, None, LIGHTPANDA_DISCOVERY_TIMEOUT).await {
...
fn lightpanda_launch_error(message: &str, logs: &LaunchLogBuffer, last_probe_error: Option<&str>) -> String {
