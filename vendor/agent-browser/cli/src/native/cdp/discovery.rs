...
pub async fn discover_cdp_url(host: &str, port: u16, query: Option<&str>) -> Result<String, String> {
...
async fn fetch_cdp_info(host: &str, port: u16, timeout: Duration) -> Result<BrowserVersionInfo, String> {
...
mod tests {
  ...
  fn append_query_adds_params_to_url_without_query() {
    ...
    assert_eq!(result, "ws://127.0.0.1:9222/devtools/browser/abc?mode=Hello");
  }
  ...
  fn append_query_merges_with_existing_query() {
    ...
    let result = append_query(url, Some("mode=Hello"));
    assert_eq!(result, "ws://127.0.0.1:9222/devtools/browser/abc?token=xyz&mode=Hello");
  }
  ...
  fn append_query_handles_multiple_params() {
    ...
    assert_eq!(result, "ws://127.0.0.1:9222/devtools/browser/abc?mode=Hello&token=abc");
  }
  ...
  async fn discover_preserves_query_params() {
    ...
    let ws_url = discover_cdp_url("127.0.0.1", port, Some("mode=Hello")).await.unwrap();
