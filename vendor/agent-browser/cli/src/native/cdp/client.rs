...
impl CdpClient {
  ...
  pub async fn connect_with_headers(url: &str, headers: Option<Vec<(String, String)>>) -> Result<Self, String> {
    let mut request = url
      .into_client_request()
    ...
    let (ws_stream, _) = tokio_tungstenite::connect_async_with_config(request, Some(ws_config), false)
      .await
  ...
  pub async fn send_command(
    ...
    let json = serde_json::to_string(&cmd).map_err(|e| format!("Failed to serialize CDP command: {}", e))?;
  ...
  pub async fn send_command_typed<P: serde::Serialize, R: serde::de::DeserializeOwned>(
    ...
    let params_value = serde_json::to_value(params).map_err(|e| format!("Failed to serialize params: {}", e))?;
    let result = self.send_command(method, Some(params_value), session_id).await?;
    serde_json::from_value(result).map_err(|e| format!("Failed to deserialize CDP response for {}: {}", method, e))
  }
