...
pub async fn set_offline(client: &CdpClient, session_id: &str, offline: bool) -> Result<(), String> {
  client
    .send_command(
...
impl DomainFilter {
  ...
  pub fn check_url(&self, url: &str) -> Result<(), String> {
    ...
    if self.is_allowed(hostname) {
      ...
      Err(format!("Domain '{}' is not in the allowed domains list", hostname))
    }
  }
}
...
pub async fn sanitize_existing_pages(client: &CdpClient, pages: &[super::browser::PageInfo], filter: &DomainFilter) {
  for page in pages {
...
pub fn format_console_arg(arg: &Value) -> Option<String> {
  ...
  if let Some(preview) = arg.get("preview") {
    ...
    if matches!(preview_subtype, Some("map" | "set" | "weakmap" | "weakset")) {
      return arg.get("description").and_then(|v| v.as_str()).map(|s| s.to_string());
    }
    let is_array = subtype == Some("array") || preview_subtype == Some("array");
    if let Some(props) = preview.get("properties").and_then(|v| v.as_array()) {
      let overflow = preview.get("overflow").and_then(|v| v.as_bool()).unwrap_or(false);
  ...
  arg.get("description").and_then(|v| v.as_str()).map(|s| s.to_string())
}
...
pub fn format_console_args(args: &[Value]) -> String {
  args.iter().filter_map(format_console_arg).collect::<Vec<_>>().join(" ")
}
