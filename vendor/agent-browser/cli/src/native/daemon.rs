...
async fn run_socket_server(
  ...
  let state: std::sync::Arc<tokio::sync::Mutex<DaemonState>> = std::sync::Arc::new(tokio::sync::Mutex::new(
...
async fn shutdown_signal() {
  #[cfg(unix)]
  {
    ...
    let mut sigterm = match signal::unix::signal(signal::unix::SignalKind::terminate()) {
      Ok(s) => s,
      Err(e) => {
        let _ = writeln!(std::io::stderr(), "Failed to install SIGTERM handler: {}", e);
        process::exit(1);
      }
    };
...
mod tests {
  ...
  fn test_child_try_wait_detects_exit_without_sigchld_handler() {
    ...
    match child.try_wait() {
      Ok(Some(status)) => {
        assert!(!status.success(), "child exited with code 42, should not be success");
      }
  ...
  async fn test_idle_timeout_fires_despite_drain_interval() {
    ...
    let exited = tokio::time::timeout(Duration::from_secs(5), async {
      let mut idle_sleep_pin = Some(Box::pin(tokio::time::sleep(Duration::from_millis(idle_timeout_ms))));
