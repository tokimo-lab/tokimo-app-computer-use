...
fn get_encryption_key() -> Result<Vec<u8>, String> {
  ...
  if key_file.exists() {
    let hex = fs::read_to_string(&key_file).map_err(|e| format!("Failed to read encryption key file: {}", e))?;
...
fn ensure_encryption_key() -> Result<Vec<u8>, String> {
  ...
  let key_file = get_key_file_path();
  fs::write(&key_file, format!("{}\n", key_hex)).map_err(|e| format!("Failed to write encryption key: {}", e))?;
  #[cfg(unix)]
  {
    use std::os::unix::fs::PermissionsExt;
...
fn encrypt_profile(profile: &AuthProfile) -> Result<String, String> {
  ...
  let cipher = Aes256Gcm::new_from_slice(&key).map_err(|e| format!("Encryption key error: {}", e))?;

  let plaintext = serde_json::to_string(profile).map_err(|e| format!("Failed to serialize profile: {}", e))?;
...
fn decrypt_profile(data: &[u8]) -> Result<AuthProfile, String> {
  let text = std::str::from_utf8(data)
    .map_err(|_| "Profile is not valid UTF-8 -- it may use an older incompatible binary format".to_string())?;
  ...
  if let Ok(payload) = serde_json::from_str::<EncryptedPayload>(text) {
    ...
    let cipher = Aes256Gcm::new_from_slice(&key).map_err(|e| format!("Decryption key error: {}", e))?;
    let plaintext = cipher
      .decrypt(aes_gcm::Nonce::from_slice(&iv), combined.as_slice())
      .map_err(|e| format!("Decryption failed: {}", e))?;

    let json_str = String::from_utf8(plaintext).map_err(|e| format!("Decrypted data is not valid UTF-8: {}", e))?;
...
pub fn credentials_set(name: &str, username: &str, password: &str, url: Option<&str>) -> Result<Value, String> {
...
mod tests {
  ...
  fn test_auth_profile_serialization() {
    ...
    assert_eq!(parsed.submit_selector, Some("button[type=submit]".to_string()));
