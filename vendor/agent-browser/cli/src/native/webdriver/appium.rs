...
impl AppiumManager {
  ...
  pub async fn create_ios_session(
    ...
    let caps = Self::build_ios_capabilities(self.device_udid.as_deref(), device_name, platform_version);
    self.client.create_session(caps).await
  }

  pub async fn tap(&self, x: f64, y: f64) -> Result<(), String> {
    let sid = self.client.session_id_pub().ok_or("No active session")?.to_string();
    let actions = json!({
        "actions": [{
            "type": "pointer",
            "id": "finger1",
...
mod tests {
  ...
  fn test_ios_capabilities_use_vendor_prefix() {
    let caps = AppiumManager::build_ios_capabilities(Some("TEST-UDID-123"), Some("iPhone 16 Pro"), Some("18.5"));
