...
pub fn list_simulators() -> Result<Vec<IosDevice>, String> {
  ...
  let parsed: Value = serde_json::from_str(&json_str).map_err(|e| format!("Failed to parse simctl: {}", e))?;
    ...
    for (runtime, device_list) in device_map {
      if let Some(arr) = device_list.as_array() {
        for device in arr {
          ...
          let udid = device.get("udid").and_then(|v| v.as_str()).unwrap_or("").to_string();
          let state = device.get("state").and_then(|v| v.as_str()).unwrap_or("").to_string();
...
pub fn select_device(device_name: Option<&str>, udid: Option<&str>) -> Result<IosDevice, String> {
  ...
  let iphone_devices: Vec<&IosDevice> = devices.iter().filter(|d| d.name.starts_with("iPhone")).collect();

  if iphone_devices.is_empty() {
    return devices.into_iter().next().ok_or("No iOS simulators found".to_string());
  }
