...
impl WebDriverClient {
  ...
  pub async fn navigate(&self, url: &str) -> Result<(), String> {
    ...
    self
      .post(&format!("/session/{}/url", sid), &json!({ "url": url }))
      .await?;
    Ok(())
  }
  ...
  pub async fn get_url(&self) -> Result<String, String> {
    ...
    Ok(response.get("value").and_then(|v| v.as_str()).unwrap_or("").to_string())
  }
  ...
  pub async fn click_element(&self, element_id: &str) -> Result<(), String> {
    let sid = self.session_id()?.to_string();
    self
      .post(&format!("/session/{}/element/{}/click", sid, element_id), &json!({}))
      .await?;
    Ok(())
  }
  ...
  pub async fn clear_element(&self, element_id: &str) -> Result<(), String> {
    let sid = self.session_id()?.to_string();
    self
      .post(&format!("/session/{}/element/{}/clear", sid, element_id), &json!({}))
      .await?;
    Ok(())
  }
  ...
  pub async fn back(&self) -> Result<(), String> {
    let sid = self.session_id()?.to_string();
    self.post(&format!("/session/{}/back", sid), &json!({})).await?;
    Ok(())
  }

  pub async fn forward(&self) -> Result<(), String> {
    let sid = self.session_id()?.to_string();
    self.post(&format!("/session/{}/forward", sid), &json!({})).await?;
    Ok(())
  }

  pub async fn refresh(&self) -> Result<(), String> {
    let sid = self.session_id()?.to_string();
    self.post(&format!("/session/{}/refresh", sid), &json!({})).await?;
    Ok(())
  }
  ...
  pub async fn execute_actions(&self, session_id: &str, actions: &Value) -> Result<(), String> {
    self.post(&format!("/session/{}/actions", session_id), actions).await?;
    Ok(())
  }
...
async fn http_request(method: &str, url: &str, body: Option<&Value>) -> Result<Value, String> {
  ...
  let json_body =
    if body_part.contains('\n') && body_part.chars().next().map(|c| c.is_ascii_hexdigit()).unwrap_or(false) {
