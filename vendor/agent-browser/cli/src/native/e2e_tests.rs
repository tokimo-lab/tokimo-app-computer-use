...
async fn create_storage_state_with_cookie(path: &str, cookie_name: &str, cookie_value: &str) {
  ...
  let resp = execute_command(&json!({ "id": "4", "action": "state_save", "path": path }), &mut state).await;
  assert_success(&resp);
...
async fn spawn_fake_daemon_socket(
  ...
  let listener = tokio::net::UnixListener::bind(&socket_path).expect("fake daemon socket should bind");
  ...
  tokio::spawn(async move {
    ...
    let _ = stream.write_all(br#"{"success":true,"data":{"ok":true}}"#).await;
...
async fn e2e_launch_navigate_evaluate_close() {
  ...
  let resp = execute_command(&json!({ "id": "1", "action": "launch", "headless": true }), &mut state).await;
  assert_success(&resp);
...
async fn e2e_runtime_stream_enable_before_launch_attaches_and_disables() {
  ...
  while tokio::time::Instant::now() < deadline {
    ...
    let parsed: Value = serde_json::from_str(message.to_text().expect("text message should be readable"))
      .expect("runtime stream payload should be valid JSON");
    if parsed.get("type") == Some(&json!("status")) && parsed.get("connected") == Some(&json!(true)) {
...
async fn e2e_stream_command_requires_same_origin_before_daemon_relay() {
  ...
  guard.set(
    "AGENT_BROWSER_SOCKET_DIR",
    socket_dir.path().to_str().expect("socket dir should be utf-8"),
  );
  guard.set("AGENT_BROWSER_SESSION", "x");

  let mut state = DaemonState::new();
  let resp = execute_command(&json!({ "id": "1", "action": "stream_enable", "port": 0 }), &mut state).await;
  assert_success(&resp);
  let port = get_data(&resp)["port"]
    .as_u64()
    .expect("stream enable should report the bound port");
  ...
  assert!(
    response.contains(&format!("Access-Control-Allow-Origin: http://localhost:{port}")),
  ...
  let resp = execute_command(&json!({ "id": "2", "action": "stream_disable" }), &mut state).await;
  assert_success(&resp);
}
...
async fn e2e_snapshot_and_click_ref() {
  ...
  assert!(snapshot.contains("Example Domain"), "Snapshot should contain heading");
  assert!(snapshot.contains("ref=e1"), "Snapshot should have ref e1");
  assert!(snapshot.contains("ref=e2"), "Snapshot should have ref e2");
  assert!(snapshot.contains("link"), "Snapshot should have a link element");

  // Click the link by ref (e2 is the "More information..." link)
  let resp = execute_command(&json!({ "id": "4", "action": "click", "selector": "e2" }), &mut state).await;
  assert_success(&resp);
...
async fn e2e_screenshot() {
  ...
  assert!(metadata.len() > 1000, "Screenshot should be non-trivial size");
...
async fn e2e_form_interaction() {
  ...
  let resp = execute_command(&json!({ "id": "2", "action": "navigate", "url": html }), &mut state).await;
  assert_success(&resp);
  ...
  assert!(snap.contains("John Doe"), "Snapshot should show filled value");
...
async fn e2e_tabs() {
  ...
  .await;
  assert_success(&resp);
  assert_eq!(get_data(&resp)["tabId"], "t2", "New tab should have tabId t2");
  ...
  // Switch to first tab
  let resp = execute_command(&json!({ "id": "6", "action": "tab_switch", "tabId": "t1" }), &mut state).await;
  assert_success(&resp);
  ...
  // Close second tab
  let resp = execute_command(&json!({ "id": "8", "action": "tab_close", "tabId": "t2" }), &mut state).await;
  assert_success(&resp);
...
async fn e2e_tab_ids_not_reused() {
  ...
  // Close tab 2
  let resp = execute_command(&json!({ "id": "5", "action": "tab_close", "tabId": "t2" }), &mut state).await;
  assert_success(&resp);
  ...
  let ids: Vec<String> = tabs.iter().map(|t| t["tabId"].as_str().unwrap().to_string()).collect();
...
async fn e2e_tab_close_with_tab_id_closes_active_tab() {
  ...
  let resp = execute_command(&json!({ "id": "4", "action": "tab_close", "tabId": "t2" }), &mut state).await;
  assert_success(&resp);
...
async fn e2e_tab_new_with_label_can_be_switched_and_closed() {
  ...
  let resp = execute_command(&json!({ "id": "5", "action": "tab_switch", "tabId": "t1" }), &mut state).await;
  assert_success(&resp);
...
async fn e2e_tab_switch_rejects_bare_integer() {
  ...
  let resp = execute_command(&json!({ "id": "2", "action": "tab_switch", "tabId": "2" }), &mut state).await;
  assert_eq!(
    resp.get("success").and_then(|v| v.as_bool()),
    Some(false),
...
async fn e2e_wait() {
  ...
  let resp = execute_command(&json!({ "id": "5", "action": "wait", "timeout": 200 }), &mut state).await;
  assert_success(&resp);
  assert!(
...
async fn e2e_navigate_same_url_twice_should_not_hang() {
  ...
  assert!(elapsed < 5, "Second navigation should not hang, but took {}s", elapsed);
  assert_success(&resp);
...
async fn e2e_hover_scroll_press() {
  ...
  // Hover
  let resp = execute_command(&json!({ "id": "3", "action": "hover", "selector": "#btn" }), &mut state).await;
  assert_success(&resp);

  // Scroll
  let resp = execute_command(&json!({ "id": "4", "action": "scroll", "y": 500 }), &mut state).await;
  assert_success(&resp);
  ...
  // Press key
  let resp = execute_command(&json!({ "id": "6", "action": "press", "key": "Enter" }), &mut state).await;
  assert_success(&resp);
...
async fn e2e_mouse_down_move_up_preserves_drag_state() {
  ...
  let initial_left = start["left"].as_i64().expect("target left should be numeric");
  ...
  let resp = execute_command(&json!({ "id": "7", "action": "mouseup", "button": "left" }), &mut state).await;
  assert_success(&resp);
  ...
  let events = probe["events"].as_array().expect("drag probe should expose events");
  assert!(
...
async fn e2e_mouse_drag_reaches_pointer_capture_target() {
  ...
  assert!(
    events.iter().any(|event| {
      event["type"] == "pointerup" && event["phase"] == "up" && event["hadCapture"].as_bool() == Some(true)
    }),
...
async fn e2e_save_state_cross_domain() {
  ...
  assert!(has_domain_b, "Should include cookie from example.com: {:?}", cookies);
...
async fn e2e_phase8_commands() {
  ...
  // Focus
  let resp = execute_command(&json!({ "id": "10", "action": "focus", "selector": "#a" }), &mut state).await;
  assert_success(&resp);

  // Clear
  let resp = execute_command(&json!({ "id": "11", "action": "clear", "selector": "#a" }), &mut state).await;
  assert_success(&resp);
...
async fn e2e_error_handling() {
  ...
  // Unknown action
  let resp = execute_command(&json!({ "id": "10", "action": "nonexistent_action" }), &mut state).await;
  assert_eq!(resp["success"], false);
  assert!(resp["error"].as_str().unwrap().contains("Not yet implemented"));

  // Missing required parameter
  let resp = execute_command(&json!({ "id": "11", "action": "fill", "selector": "#x" }), &mut state).await;
  assert_eq!(resp["success"], false);
...
async fn e2e_profile_cookie_persistence() {
  let profile_dir = std::env::temp_dir().join(format!("agent-browser-e2e-profile-{}", uuid::Uuid::new_v4()));
...
async fn e2e_get_cdp_url() {
  ...
  assert_success(&resp);
  let cdp_url = get_data(&resp)["cdpUrl"].as_str().expect("cdpUrl should be a string");
  assert!(
...
async fn e2e_inspect() {
  ...
  let url = data["url"].as_str().expect("inspect url should be a string");
  assert!(
...
async fn e2e_click_stale_ref_falls_back_to_role_name() {
  ...
  assert_success(&resp);
  let snapshot = get_data(&resp)["snapshot"].as_str().unwrap();
  assert!(snapshot.contains("Replace"), "Snapshot should contain Replace button");
  assert!(snapshot.contains("Target"), "Snapshot should contain Target button");

  // Click "Replace" — this removes all DOM nodes and recreates them,
  // making the backend_node_id for "Target" stale.
  let resp = execute_command(&json!({ "id": "4", "action": "click", "selector": "e1" }), &mut state).await;
  assert_success(&resp);
  ...
  let resp = execute_command(&json!({ "id": "6", "action": "click", "selector": "e2" }), &mut state).await;
  assert_success(&resp);
  tokio::time::sleep(tokio::time::Duration::from_millis(200)).await;
...
async fn e2e_snapshot_cursor_interactive() {
  ...
  let resp = execute_command(&json!({ "id": "2", "action": "setcontent", "html": html }), &mut state).await;
  assert_success(&resp);
  ...
  for line in snapshot.lines() {
    assert!(
      !(line.contains("\"Link\"")
        && (line.contains("clickable") || line.contains("focusable") || line.contains("editable"))),
...
async fn e2e_snapshot_cursor_many_elements() {
  ...
  let mut html = String::from("<html><body>");
  for i in 1..=100 {
    html.push_str(&format!("<div style='cursor:pointer' onclick='x()'>Item {}</div>", i,));
  }
  html.push_str("</body></html>");
...
async fn e2e_auth_login_waits_for_delayed_spa_form_render() {
  ...
  let launch = execute_command(&json!({ "id": "1", "action": "launch", "headless": true }), &mut state).await;
...
async fn e2e_externally_opened_tab_detected() {
  ...
  let new_tab = tabs
    .iter()
    .find(|t| t["url"].as_str().is_some_and(|u| u.starts_with("data:text/html")));
  assert!(
    new_tab.is_some(),
...
async fn e2e_relaunch_on_options_change() {
  ...
  let resp = execute_command(&json!({ "id": "2", "action": "launch", "headless": true }), &mut state).await;
  assert_success(&resp);
  assert_eq!(
...
async fn e2e_stream_frame_metadata_respects_custom_viewport() {
  ...
  while tokio::time::Instant::now() < deadline {
    ...
    if parsed.get("type") == Some(&json!("frame")) {
      ...
      let (img_w, img_h) = jpeg_dimensions(&bytes).expect("frame data should be a valid JPEG with SOF marker");
...
async fn e2e_recording_inherits_viewport() {
  ...
  let resp = execute_command(&json!({ "id": "7", "action": "recording_stop" }), &mut state).await;
  assert_success(&resp);
...
async fn e2e_state_flag_restores_cookies() {
  let state_path = std::env::temp_dir()
    .join(format!("agent-browser-e2e-state-flag-{}.json", uuid::Uuid::new_v4()))
    .to_string_lossy()
    .to_string();
...
async fn e2e_state_flag_missing_file_fails_launch() {
  ...
  let missing_path = std::env::temp_dir()
    .join(format!("agent-browser-e2e-missing-state-{}.json", uuid::Uuid::new_v4()))
    .to_string_lossy()
    .to_string();
...
async fn e2e_state_env_restores_cookies_on_auto_launch() {
  let state_path = std::env::temp_dir()
    .join(format!("agent-browser-e2e-state-env-{}.json", uuid::Uuid::new_v4()))
    .to_string_lossy()
    .to_string();
...
async fn e2e_session_name_auto_restores_cookies() {
  ...
  let sessions_dir = dirs::home_dir().unwrap().join(".agent-browser").join("sessions");
...
async fn e2e_explicit_state_load_restores_cookies() {
  let state_path = std::env::temp_dir()
    .join(format!("agent-browser-e2e-explicit-load-{}.json", uuid::Uuid::new_v4()))
    .to_string_lossy()
    .to_string();
  ...
  // Session 2: launch clean, then explicitly load state
  {
    ...
    let resp = execute_command(&json!({ "id": "10", "action": "launch", "headless": true }), &mut state).await;
    assert_success(&resp);
    ...
    let resp = execute_command(&json!({ "id": "13", "action": "cookies_get" }), &mut state).await;
    assert_success(&resp);
    let cookies = get_data(&resp)["cookies"].as_array().unwrap();
    let found = cookies
      .iter()
...
fn react_fixture_url() -> String {
  format!("data:text/html;base64,{}", STANDARD.encode(REACT_FIXTURE_HTML))
}
...
async fn e2e_react_tree_with_enable_hook() {
  ...
  let tree = get_data(&resp)
    ...
    .unwrap_or_default()
    .to_string();
  assert!(tree.contains("App"), "Expected tree to contain 'App': {}", tree);
  assert!(tree.contains("Counter"), "Expected tree to contain 'Counter': {}", tree);
...
async fn e2e_pushstate_changes_url() {
  ...
  .await;
  assert_success(&resp);
  let url = get_data(&resp).get("url").and_then(|v| v.as_str()).unwrap_or_default();
  assert!(
