...
use super::cdp::types::{AXNode, AXProperty, AXValue, EvaluateParams, EvaluateResult, GetFullAXTreeResult};
...
pub async fn take_snapshot(
  ...
  let selector_backend_ids: Option<std::collections::HashSet<i64>> = if let Some(ref selector) = options.selector {
    let js = format!(
    ...
    if ids.is_empty() {
      return Err(format!("Could not resolve backendNodeId for selector '{}'", selector));
    }
  ...
  let (ax_params, effective_session_id) = resolve_ax_session(frame_id, session_id, iframe_sessions);
  ...
  let cursor_elements: HashMap<i64, CursorElementInfo> = find_cursor_interactive_elements(client, session_id)
    .await
    .unwrap_or_default();
  ...
  if options.urls {
    ...
    if !link_nodes.is_empty() {
      ...
      let resolved: Vec<(usize, Option<String>)> = futures_util::future::join_all(resolve_futs).await;
      ...
      let href_futs: Vec<_> = resolved
        ...
        .collect();
      let hrefs: Vec<(usize, Option<String>)> = futures_util::future::join_all(href_futs).await;
  ...
  if frame_id.is_none() {
    ...
    for node in tree_nodes.iter() {
      ...
      if let Ok(child_fid) = resolve_iframe_frame_id(client, session_id, bid).await {
        ...
        .await
        {
          if !child_text.is_empty() && child_text != "(empty page)" && child_text != "(no interactive elements)" {
    ...
    for (ref_id, child_text) in iframe_snapshots {
      ...
      if let Some(pos) = output.find(&marker) {
        ...
        let indented_child: String = child_text.lines().map(|line| format!("{}{}\n", prefix, line)).collect();
...
async fn resolve_iframe_frame_id(client: &CdpClient, session_id: &str, backend_node_id: i64) -> Result<String, String> {
...
"#;
  ...
  for (i, elem) in elements.iter().enumerate() {
    ...
    let has_cursor_pointer = elem.get("hasCursorPointer").and_then(|v| v.as_bool()).unwrap_or(false);
    let has_on_click = elem.get("hasOnClick").and_then(|v| v.as_bool()).unwrap_or(false);
    let has_tab_index = elem.get("hasTabIndex").and_then(|v| v.as_bool()).unwrap_or(false);
    let is_editable = elem.get("isEditable").and_then(|v| v.as_bool()).unwrap_or(false);
...
fn build_tree(nodes: &[AXNode]) -> (Vec<TreeNode>, Vec<usize>) {
  ...
  for (i, node) in nodes.iter().enumerate() {
    ...
    let (level, checked, expanded, selected, disabled, required) = extract_properties(&node.properties);
  ...
  for i in 0..tree_nodes.len() {
    ...
    while start < children_indices.len() {
      ...
      let mut end = start + 1;
      while end < children_indices.len() && tree_nodes[children_indices[end]].role == "StaticText" {
        end += 1;
      }
...
fn render_tree(nodes: &[TreeNode], idx: usize, indent: usize, output: &mut String, options: &SnapshotOptions) {
  let node = &nodes[idx];
...
mod tests {
  ...
  fn test_compact_tree_radio_checkbox() {
    ...
    assert!(result.contains("radio \"Single unit\""), "radio should be kept");
    assert!(result.contains("checkbox \"I agree\""), "checkbox should be kept");
    assert!(result.contains("button \"Submit\""), "button should be kept");
  }
  ...
  fn test_cross_origin_iframe_uses_dedicated_session() {
    ...
    let (params, session) = resolve_ax_session(Some(iframe_frame_id), parent_session, &iframe_sessions);
