...
impl TestKeyGuard {
  fn new() -> Self {
    let lock = super::auth::AUTH_TEST_MUTEX.lock().unwrap_or_else(|e| e.into_inner());
...
fn minimal_command(action: &str, id: &str) -> Value {
  ...
  match action {
    ...
    | "gettext" | "getattribute" | "isvisible" | "isenabled" | "ischecked" | "focus" | "clear" | "selectall"
    | "scrollintoview" | "dispatch" | "highlight" | "tap" | "boundingbox" | "innertext" | "innerhtml"
    | "inputvalue" | "setvalue" | "count" | "find" | "nth" | "getbytext" | "getbylabel" | "getbyplaceholder"
    ...
    "viewport" | "user_agent" | "set_media" | "timezone" | "locale" | "geolocation" | "permissions" | "device" => {
...
async fn test_all_documented_actions_are_handled() {
  ...
  for (i, action) in DOCUMENTED_ACTIONS.iter().enumerate() {
    ...
    assert!(result.get("id").is_some(), "Action '{}': response missing 'id'", action);
...
async fn test_addscript_and_addinitscript_separate_dispatch() {
  ...
  assert!(!err1.contains("Not yet implemented"), "addscript should be handled");
  ...
  assert!(!err2.contains("Not yet implemented"), "addinitscript should be handled");
}
...
async fn test_addstyle_supports_content_and_url() {
  ...
  let cmd2 = json!({ "action": "addstyle", "id": "style-2", "url": "https://example.com/style.css" });
  let result2 = execute_command(&cmd2, &mut state).await;
  let err2 = result2["error"].as_str().unwrap_or("");
