...
pub async fn resolve_element_center(
  ...
  if let Some(ref_id) = parse_ref(selector_or_ref) {
    let entry = ref_map.get(&ref_id).ok_or_else(|| format!("Unknown ref: {}", ref_id))?;

    let effective_session_id = resolve_frame_session(entry.frame_id.as_deref(), session_id, iframe_sessions);
    ...
    if node_role == role && node_name == name {
      if match_count == nth_index {
        return node
          ...
          .ok_or_else(|| format!("AX node has no backendDOMNodeId for role={} name={}", role, name));
      }
      match_count += 1;
    }
  }

  Err(format!("Could not locate element with role={} name={}", role, name))
}
...
async fn resolve_by_selector(client: &CdpClient, session_id: &str, selector: &str) -> Result<(f64, f64), String> {
...
pub async fn get_element_text(
  ...
  let (object_id, effective_session_id) =
    resolve_element_object_id(client, session_id, ref_map, selector_or_ref, iframe_sessions).await?;

  let result: EvaluateResult = client
    .send_command_typed(
      "Runtime.callFunctionOn",
      &CallFunctionOnParams {
        function_declaration: "function() { return this.innerText || this.textContent || ''; }".to_string(),
...
pub async fn is_element_enabled(
  ...
  Ok(result.result.value.and_then(|v| v.as_bool()).unwrap_or(true))
}
...
pub async fn get_element_input_value(
  ...
  let result: EvaluateResult = client
    .send_command_typed(
      "Runtime.callFunctionOn",
      &CallFunctionOnParams {
        function_declaration: "function() { return typeof this.value === 'string' ? this.value : ''; }".to_string(),
        object_id: Some(object_id),
        arguments: None,
        return_by_value: Some(true),
        await_promise: Some(false),
...
pub async fn get_element_count(client: &CdpClient, session_id: &str, selector: &str) -> Result<i64, String> {
...
mod tests {
  ...
  fn test_build_selector_js_xpath() {
    ...
    assert!(js.contains(
      "document.evaluate(\"//button[@id='ok']\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null)"
    ));
  ...
  fn test_build_count_elements_js_xpath() {
    ...
    assert!(js.contains(
      "document.evaluate(\"//li\", document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotLength"
    ));
  ...
  fn test_cross_origin_element_uses_dedicated_session() {
    ...
    iframe_sessions.insert("cross-origin-frame".to_string(), "iframe-session".to_string());

    let session = resolve_frame_session(Some("cross-origin-frame"), "parent-session", &iframe_sessions);
  ...
  fn test_same_origin_element_uses_parent_session() {
    ...
    let session = resolve_frame_session(Some("same-origin-frame"), "parent-session", &iframe_sessions);
