...
use super::cdp::types::{
  AttachToTargetParams, AttachToTargetResult, CloseTargetParams, CreateTargetParams, CreateTargetResult, EvaluateParams,
};
...
fn parse_origin_storage(data: &Value) -> Option<OriginStorage> {
  ...
  let origin = data.get("origin").and_then(|v| v.as_str()).unwrap_or("").to_string();
...
async fn eval_origin_storage(client: &CdpClient, session_id: &str, origin_js: &str) -> Option<OriginStorage> {
  let result = client
...
async fn collect_storage_via_temp_target(
  ...
  let _ = client
    .send_command_typed::<_, Value>("Target.closeTarget", &CloseTargetParams { target_id }, None)
    .await;
...
async fn collect_storage_in_target(
  ...
  client.send_command_no_params("Page.enable", Some(temp_session)).await?;
  client
  ...
  for target_origin in origins {
    ...
    if client
      .send_command("Page.navigate", Some(json!({ "url": nav_url })), Some(temp_session))
      .await
      .is_err()
    {
      continue;
      ...
      match tokio::time::timeout(tokio::time::Duration::from_secs(2), event_rx.recv()).await {
        Ok(Ok(evt)) if evt.session_id.as_deref() == Some(temp_session) => {
          if evt.method == "Fetch.requestPaused" {
            if let Some(request_id) = evt.params.get("requestId").and_then(|v| v.as_str()) {
              let _ = client
                .send_command(
...
pub async fn save_state(
  ...
  if !all_origins.is_empty() {
    ...
    if let Ok(temp_origins) = collect_storage_via_temp_target(client, &remaining, origin_js).await {
      origins.extend(temp_origins);
    }
  }

  let state = StorageState { cookies, origins };
  let json_str = serde_json::to_string_pretty(&state).map_err(|e| format!("Failed to serialize state: {}", e))?;
  ...
  let mut save_path = match path {
    ...
    None => {
      ...
      dir
        .join(format!("{}-{}.json", name, session_id_str))
        .to_string_lossy()
        .to_string()
    }
  };
  ...
  if let Ok(key) = std::env::var("AGENT_BROWSER_ENCRYPTION_KEY") {
    ...
    fs::write(&save_path, &encrypted).map_err(|e| format!("Failed to write state to {}: {}", save_path, e))?;
  } else {
    fs::write(&save_path, &json_str).map_err(|e| format!("Failed to write state to {}: {}", save_path, e))?;
  }
...
pub async fn load_state(client: &CdpClient, session_id: &str, path: &str) -> Result<(), String> {
  ...
  for origin in &state.origins {
    ...
    client
      .send_command("Page.navigate", Some(json!({ "url": navigate_url })), Some(session_id))
      .await?;
...
pub fn find_auto_state_file(session_name: &str) -> Option<String> {
  ...
  if let Ok(entries) = fs::read_dir(&dir) {
    for entry in entries.flatten() {
      let path = entry.path();
      let fname = path.file_name().unwrap_or_default().to_string_lossy().to_string();
      let is_match = fname.starts_with(&prefix) && (fname.ends_with(".json") || fname.ends_with(".json.enc"));
      if !is_match {
        continue;
      }
