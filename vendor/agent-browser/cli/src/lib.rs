// Vendored copy of vercel-labs/agent-browser; suppress lints on upstream code.
#![allow(warnings, clippy::all, clippy::pedantic, clippy::nursery, clippy::cargo)]
...
use connection::{
  cleanup_stale_files, ensure_daemon, get_socket_dir, is_pid_alive, send_command, walk_daemons, DaemonOptions,
};
...
fn serialize_json_value(value: &serde_json::Value) -> String {
  serde_json::to_string(value)
    .unwrap_or_else(|_| r#"{"success":false,"error":"Failed to serialize JSON response"}"#.to_string())
}
...
fn run_profiles(json_mode: bool) {
  ...
  if json_mode {
    ...
    println!("{} ({}):\n", color::bold("Chrome profiles"), user_data_dir.display());
    for p in &profiles {
      println!(
        "  {}  {}",
...
fn run_session(args: &[String], session: &str, json_mode: bool) {
  ...
  match subcommand {
    Some("list") => {
      let sessions: Vec<String> = walk_daemons().sessions.into_iter().map(|s| s.name).collect();
...
fn run_close_all(flags: &Flags) {
  ...
  let sessions: Vec<(String, u32)> = inventory.sessions.iter().map(|s| (s.name.clone(), s.pid)).collect();
...
pub fn run(args: Vec<String>) {
  ...
  if clean.first().map(|s| s.as_str()) == Some("dashboard") {
    match clean.get(1).map(|s| s.as_str()) {
      ...
      Some(unknown) => {
        eprintln!("{} Unknown dashboard subcommand: {}", color::error_indicator(), unknown);
        exit(1);
      }
    }
  }
  ...
  if daemon_result.already_running {
    let ignored_flags: Vec<&str> = [
      ...
      if flags.cli_profile { Some("--profile") } else { None },
      if flags.cli_state { Some("--state") } else { None },
      ...
      if flags.cli_proxy { Some("--proxy") } else { None },
  ...
  if flags.auto_connect && !daemon_result.already_running {
    ...
    let err = match send_command(launch_cmd, &flags.session) {
      ...
      Ok(resp) => Some(resp.error.unwrap_or_else(|| "Auto-connect failed".to_string())),
  ...
  if let Some(ref cdp_value) = flags.cdp {
      ...
      || cdp_value.starts_with("https://")
    {
      ...
      let cdp_port: u16 = match cdp_value.parse::<u32>() {
        ...
        Ok(p) => p as u16,
        Err(_) => {
          let msg = format!("Invalid CDP value: '{}' is not a valid port number or URL", cdp_value);
          if flags.json {
            print_json_error(&msg);
          } else {
            eprintln!("{} {}", color::error_indicator(), msg);
    ...
    && !flags.auto_connect
  {
    ...
    match send_command(launch_cmd, &flags.session) {
      Ok(resp) if !resp.success => {
        ...
        let error_msg = resp.error.unwrap_or_else(|| "Browser launch failed".to_string());
        if flags.json {
      ...
      Err(e) => {
        if flags.json {
          ...
          eprintln!("{} Could not configure browser: {}", color::error_indicator(), e);
        }
        exit(1);
      }
      Ok(_) => {
    ...
    let parsed = match parse_command(cmd_args, flags) {
      ...
      Err(e) => {
        ...
        if flags.json {
          ...
          eprintln!("{} Command {}: {}", color::error_indicator(), i + 1, e.format());
          if bail {
            exit(1);
          }
        }
