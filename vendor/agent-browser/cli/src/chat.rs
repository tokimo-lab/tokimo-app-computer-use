...
pub fn run_chat(flags: &Flags, message: Option<String>) {
  ...
  let model = flags.model.clone().unwrap_or_else(|| DEFAULT_MODEL.to_string());
  ...
  match message {
    Some(msg) => {
      rt.block_on(run_single_turn(&flags.session, &model, &msg, verbosity, flags.json));
    }
    None if !is_tty => {
      ...
      rt.block_on(run_single_turn(&flags.session, &model, input, verbosity, flags.json));
    }
    None => {
      rt.block_on(run_interactive(&flags.session, &model, verbosity, flags.json));
    }
  }
}

async fn run_single_turn(session: &str, model: &str, message: &str, verbosity: Verbosity, json_mode: bool) {
  let mut openai_messages: Vec<Value> = vec![json!({"role": "system", "content": chat::get_system_prompt()})];
...
async fn run_interactive(session: &str, model: &str, verbosity: Verbosity, json_mode: bool) {
  ...
  loop {
    ...
    if total_chars > chat::COMPACT_THRESHOLD_CHARS && openai_messages.len() > chat::KEEP_RECENT_MESSAGES + 2 {
      let split = chat::find_safe_split(&openai_messages, chat::KEEP_RECENT_MESSAGES);
      let to_summarize = &openai_messages[1..split];
      if let Some(summary) = chat::summarize_for_compaction(client, &url, &api_key, model, to_summarize).await {
    ...
    let success = run_chat_turn(session, model, &mut openai_messages, verbosity, json_mode).await;
...
async fn run_chat_turn(
  ...
  let api_key = match std::env::var("AI_GATEWAY_API_KEY") {
    ...
    Err(_) => {
      if json_mode {
        println!("{}", json!({"success": false, "error": "AI_GATEWAY_API_KEY not set"}));
      } else {
  ...
  for _step in 0..50 {
      ...
      .await
    {
      ...
      Err(e) => {
        if json_mode {
          ...
          eprintln!("\n{} Gateway request failed: {}", color::error_indicator(), e);
        }
        return false;
      }
    };
    ...
    let (text_chunks, tool_calls) = parse_gateway_stream(gw_response, verbosity, json_mode).await;
    ...
    if !text_chunks.is_empty() {
      ...
      if let Some(last) = openai_messages.last() {
        if last.get("role").and_then(|r| r.as_str()) == Some("assistant") && last.get("tool_calls").is_some() {
          content = json!(text);
        }
      }
    ...
    if text_chunks.is_empty() {
      ...
      if let Some(last) = openai_messages.last_mut() {
        if last.get("role").and_then(|r| r.as_str()) == Some("assistant") && last.get("tool_calls").is_none() {
    ...
    for (tc_id, _tc_name, tc_args) in &tool_calls {
      ...
      let result = match tokio::time::timeout(tool_timeout, chat::execute_chat_tool(session, command)).await {
        Ok(r) => r,
...
async fn parse_gateway_stream(
  ...
  let mut tool_call_args: std::collections::HashMap<usize, (String, String, String)> = std::collections::HashMap::new();
...
fn collect_tool_calls(
  ...
  indices.sort();
  indices.into_iter().filter_map(|idx| map.remove(&idx)).collect()
}
