...
fn read_config_file(path: &Path) -> Option<Config> {
  ...
  match serde_json::from_str::<Config>(&content) {
    Ok(mut config) => {
      config.idle_timeout =
        parse_idle_timeout_value(config.idle_timeout.take(), &format!("config file {}", path.display()));
      Some(config)
    }
    Err(e) => {
      eprintln!(
...
pub fn load_config(args: &[String]) -> Result<Config, String> {
  let explicit = extract_config_path(args).map(|p| ("--config", p)).or_else(|| {
  ...
  if let Some((source, maybe_path)) = explicit {
    ...
    return read_config_file(&path).ok_or_else(|| format!("failed to load config from {}", path_str));
  }
...
pub fn parse_flags(args: &[String]) -> Flags {
  ...
  let mut flags = Flags {
    ...
    user_agent: env::var("AGENT_BROWSER_USER_AGENT").ok().or(config.user_agent),
    ...
    auto_connect: env_var_is_truthy("AGENT_BROWSER_AUTO_CONNECT") || config.auto_connect.unwrap_or(false),
    session_name: env::var("AGENT_BROWSER_SESSION_NAME").ok().or(config.session_name),
    annotate: env_var_is_truthy("AGENT_BROWSER_ANNOTATE") || config.annotate.unwrap_or(false),
    color_scheme: env::var("AGENT_BROWSER_COLOR_SCHEME").ok().or(config.color_scheme),
    download_path: env::var("AGENT_BROWSER_DOWNLOAD_PATH").ok().or(config.download_path),
    ...
    action_policy: env::var("AGENT_BROWSER_ACTION_POLICY").ok().or(config.action_policy),
    ...
    screenshot_dir: env::var("AGENT_BROWSER_SCREENSHOT_DIR").ok().or(config.screenshot_dir),
    ...
    no_auto_dialog: env_var_is_truthy("AGENT_BROWSER_NO_AUTO_DIALOG") || config.no_auto_dialog.unwrap_or(false),
...
mod tests {
  ...
  fn test_load_config_error_malformed_explicit() {
    ...
    let flag_args = vec!["--config".to_string(), config_path.to_string_lossy().to_string()];
