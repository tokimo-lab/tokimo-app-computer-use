...
fn main() {
  ...
  if !browser_path.exists() && !js_path.exists() {
    fs::write(&out_path, "// No protocol JSON files found in cdp-protocol/\n").unwrap();
    return;
  }
  ...
  let mut domain_types: std::collections::HashMap<String, HashSet<String>> = std::collections::HashMap::new();
...
fn resolve_ref(
  ...
  if parts.len() == 2 {
    ...
    if ref_domain == current_domain {
      ...
      if domain_types.get(ref_domain).is_some_and(|t| t.contains(ref_type)) {
        format!("super::cdp_{}::{}", to_snake_case(ref_domain), to_pascal_case(ref_type))
      } else {
  ...
  for type_def in &domain.types {
    if !type_def.enum_values.is_empty() {
      ...
      for prop in &type_def.properties {
        ...
        if recursive_fields.contains(&(domain.domain.as_str(), type_def.id.as_str(), prop.name.as_str())) {
      ...
      output.push_str(&format!("    pub type {} = Vec<serde_json::Value>;\n\n", type_def.id));
